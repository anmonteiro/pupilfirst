...
let loadCourses = (state, cursor, send) => {
  let variables =
    CoursesInfoQuery.makeVariables(
...
let showCourse = (course: course, selected, onChangeCB) => {
  let selectedClass = if (selected) {" bg-primary-100"} else {""};
  <Spread key={course.id} props={"data-course-id": course.name}>
    <div
      className={
        "w-full flex cursor-pointer hover:bg-gray-300" ++ selectedClass
      }
  ...
  <div className="w-full">
    {if (ArrayUtils.isEmpty(courses)) {
       ...
       <div className="flex flex-wrap">
         {Js.Array.map(
            ~f=course => showCourse(course, course.id == value, onChangeCB),
            courses,
          )
          ->React.array}
       </div>;
...
let reloadCoursesCB = (send, ()) => send(ReloadCourses);
    ...
    <div className="flex-1 flex flex-col">
      ...
      <div id="courses" className="mx-auto max-w-4xl w-full">
        {switch (state.courses) {
         ...
         | FullyLoaded(courses) =>
           <div> {showCourses(courses, state, value, onChange)} </div>
         }}
      </div>
      {switch (state.courses) {
       | Unloaded => React.null
