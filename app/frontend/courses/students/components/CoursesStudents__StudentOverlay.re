[%%raw {js|import "./CoursesStudents__StudentOverlay.css"|js}];
open CoursesStudents__Types;
let str = React.string;
let t = I18n.t(~scope="components.CoursesStudents__StudentOverlay");
type nonrec selectedTab =
  | Notes
  | Submissions;
type nonrec studentData =
  | Loading
...
let closeOverlay = courseId => {
  ...
  ReasonReactRouter.push(("/courses/" ++ courseId ++ "/students") ++ search);
};
...
let getStudentDetails = (studentId, setState) => {
  ...
  |> Js.Promise.then_((response: StudentDetailsQuery.t) => {
       ...
       let coachNotes =
         response.coachNotes
         ->(
             Js.Array.map(~f=(coachNote: StudentDetailsQuery.t_coachNotes) =>
               CoachNote.make(
                 ~id=coachNote.id,
                 ~note=coachNote.note,
                 ~createdAt=coachNote.createdAt->DateFns.decodeISO,
                 ~author=
                   coachNote.author->(Belt.Option.map(User.makeFromFragment)),
               )
             )
           );
       let evaluationCriteria =
         response.studentDetails.evaluationCriteria
         ->(
             Js.Array.map(
               ~f=(
                    evaluationCriterion: StudentDetailsQuery.t_studentDetails_evaluationCriteria,
                  ) =>
       ...
       let averageGrades =
         response.studentDetails.averageGrades
         ->(
             Js.Array.map(
               ~f=(
                    gradeData: StudentDetailsQuery.t_studentDetails_averageGrades,
                  ) =>
       ...
       let studentDetails =
         StudentDetails.make(
           ...
           ~levels=
             response.studentDetails.student.course.levels
             ->(Js.Array.map(~f=Level.makeFromFragment)),
           ~student=
             StudentInfo.make(
               ~id=s.id,
               ~taggings=s.taggings,
               ...
               ~droppedOutAt=
                 s.droppedOutAt->(Belt.Option.map(DateFns.decodeISO)),
               ~personalCoaches=
                 s.personalCoaches
                 ->(Js.Array.map(~f=UserProxy.makeFromFragment)),
             ),
           ~team=
             response.studentDetails.team
             ->(
                 Belt.Option.map(team =>
                   StudentDetails.makeTeam(
                     ...
                     ~students=
                       ...
                       ->(
                           Js.Array.map(
                             ~f=(
                                  s: StudentDetailsQuery.t_studentDetails_team_students,
                                ) =>
                             StudentInfo.make(
                               ~id=s.id,
                               ~taggings=s.taggings,
                               ...
                               ~droppedOutAt=
                                 s.droppedOutAt
                                 ->(Belt.Option.map(DateFns.decodeISO)),
                               ~personalCoaches=
                                 s.personalCoaches
                                 ->(
                                     Js.Array.map(
                                       ~f=UserProxy.makeFromFragment,
                                     )
                                   ),
                             )
                           )
...
let studentLevelClasses = (levelNumber, levelCompleted, currentLevelNumber) => {
  ...
  let completed =
    if (levelCompleted) {" student-overlay__student-level--completed"} else {
      ""
    };
...
let levelProgressBar = (levelId, levels, levelsCompleted) => {
  let applicableLevels =
    levels |> Js.Array.filter(~f=level => Level.number(level) != 0);
  let courseCompleted =
    applicableLevels
  ...
  <div className="mb-8">
    <div className="flex justify-between items-end">
      <h6 className="text-sm font-semibold">
        {t("level_progress") |> str}
      </h6>
      {if (courseCompleted) {
         <p className="text-green-600 font-semibold">
           {{js|🎉|js} |> str}
           <span className="text-xs ml-px">
             {t("course_completed") |> str}
           </span>
         </p>;
       } else {
         React.null;
...
let coachInfo = studentDetails => {
  let coaches =
    studentDetails->StudentDetails.student->StudentInfo.personalCoaches;
  ...
  | Some(team) =>
    <div className="block mb-8">
      ...
      {team
       ...
       ->(Js.Array.filter(~f=student => StudentInfo.id(student) != studentId))
       ->(
...
let inactiveWarning = student => {
  let warning =
    ...
    | (None, Some(endsAt)) =>
      if (endsAt->DateFns.isPast) {
        Some(
          t(
            ~variables=[|
              (
                "date",
                endsAt->(DateFns.formatPreset(~short=true, ~year=true, ())),
              ),
            |],
            "access_ended_at",
          ),
  ...
  <div className="text-center mt-1">
    <ClickToCopy
      ...
      <span className="font-semibold text-sm underline text-primary-500">
        {({js|#|js} ++ StudentInfo.user(student)->UserDetails.id)->str}
      </span>
    </ClickToCopy>
    <ClickToCopy
      copy={StudentInfo.id(student)}
      className="ml-2 inline-block hover:text-primary-500">
      <span className="text-xs"> "Student ID "->str </span>
      <span className="font-semibold text-sm underline text-primary-500">
        {({js|#|js} ++ StudentInfo.id(student))->str}
      </span>
    </ClickToCopy>
  </div>;
[@react.component]
         ...
         <div
           ...
           <div className="student-overlay__student-details relative pb-8">
             <button
               ...
               onClick={_ =>
                 closeOverlay(StudentDetails.courseId(studentDetails))
               }
       ...
       <div className="flex flex-col md:flex-row md:h-screen">
         <div className="w-full md:w-2/5 bg-white p-4 md:p-8 2xl:p-16">
           ...
           {SkeletonLoading.multiple(
              ~count=2,
              ~element=SkeletonLoading.userDetails(),
            )}
         </div>
         <div
