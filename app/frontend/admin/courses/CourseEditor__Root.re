[%%mel.raw {js|import "./CourseEditor__Root.css"|js}];

[@mel.module "../../shared/images/add-new-course.svg"]
...
type nonrec status = [ | `Active | `Ended | `Archived];
...
let reducer = (state, action) =>
  switch (action) {
  | UpdateSelectedCourse(selectedCourse) => {...state, selectedCourse}
  | SetSearchString(string) => {
      ...state,
      filter: {
        ...state.filter,
  ...
  | 
    ...
    let courses =
      ...
      | LoadingMore =>
        Js.Array.concat(newCourses, ~other=Pagination.toArray(state.courses))
...

    {
      ...
      totalEntriesCount,
      schoolStats:
        Belt.Option.getWithDefault(schoolSummary, state.schoolStats),
    };
  | UpdateCourse(course) =>
    let newCourses =
      Pagination.update(state.courses, Course.updateList(course));
...
let courseLinks = course => {
  ...
  [|
    ...
    courseLink(
      baseUrl ++ "/coaches",
      t("course_links.manage_coaches"),
      "fas fa-fw fa-user fa-fw",
    ),
    courseLink(
      baseUrl ++ "/exports",
...
let loadCourses = (courseId, state, cursor, ~skipSchoolStatsLoad=true, send) => {
  ...
  |> Js.Promise.then_((response: CoursesQuery.t) => {
       let courses =
         ...
         ->(Js.Array.map(~f=rawCourse => Course.makeFromFragment(rawCourse)));
       let course =
         response.course->(Belt.Option.map(Course.makeFromFragment));
       switch (course) {
       | None => send(UpdateSelectedCourse(None))
       | Some(course) =>
         switch (
           courses->(Js.Array.find(~f=(c: Course.t) => c.id == course.id))
         ) {
       ...
       let schoolStats =
         Belt.Option.map(response.schoolStats, s =>
           Loaded({
...
let unselected = state => {
  ...
  let name =
    if (trimmedFilterString == "") {
      ...
      [|Selectable.name(trimmedFilterString)|];
    };
  let status =
    Js.Array.map(
      ~f=s => Selectable.status(s),
      Belt.Option.mapWithDefault(
        state.filter.status, [|`Active, `Ended, `Archived|], u =>
        switch (u) {
  ...
  Js.Array.concat(status, ~other=name);
};
...
let selected = state => {
  ...
  Js.Array.concat(status, ~other=selectedSearchString);
};
...
let entriesLoadedData = (totoalNotificationsCount, loadedNotificaionsCount) =>
  <div
    ...
    (
      if (totoalNotificationsCount == loadedNotificaionsCount) {
        t(
          ~variables=[|
...
let showCourse = course =>
  <Spread key={Course.id(course)} props={"data-t": Course.name(course)}>
    <div className="w-full relative">
      <div
        className="flex shadow bg-white rounded-lg flex-col justify-between h-full">
        <div>
          ...
          {ReactUtils.nullIf(
             <div className="px-4 pt-4">
               <a
                 ariaLabel={
                   (t("view_public_page") ++ " ") ++ Course.name(course)
                 }
          ...
          <p className="text-sm px-4 py-2 text-gray-600">
            {str(Course.description(course))}
          </p>
        </div>
        <div className="grid grid-cols-3 divide-x py-4 divide-gray-300 ">
          <Spread
            props={"data-t": Course.name(course) ++ {js| cohorts count|js}}>
          ...
          <Spread
            props={"data-t": Course.name(course) ++ {js| coaches count|js}}>
            <div className="flex-1 px-4">
              <p className="text-sm text-gray-500 font-medium">
                {ts("coaches")->str}
              </p>
          ...
          <Spread
            props={"data-t": Course.name(course) ++ {js| levels count|js}}>
            <div className="flex-1 px-4">
          ...
          <button
            ...
            onClick={_ =>
              ReasonReactRouter.push(
                ("/school/courses/" ++ Course.id(course)) ++ "/details",
              )
            }>
            <div>
              <FaIcon classes="far fa-edit mr-3" />
              <span className="font-semibold">
                {str(t("edit_course_details"))}
              </span>
            </div>
          </button>
        </div>
  ...
  <div className="w-full">
    {if (ArrayUtils.isEmpty(courses)) {
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-8">
         ...
         {Js.Array.map(~f=course => showCourse(course), courses)->React.array}
       </div>;
     }}
...
let raiseUnsafeFindError = id => {
  let message =
    ("Unable to be find course with id: " ++ id) ++ " in CourseEditor";
  Rollbar.error(message);
  Notification.error(
    t("notification_error_head"),
    t("notification_error_body"),
  );
...
let make = (~school) => {
  ...
  let (editorAction, selectedTab) =
    switch (url.path) {
    | ["school", "courses", "new", ..._] => (
        ShowForm(None),
        CourseEditor__Form.DetailsTab,
      )
    | ["school", "courses", courseId, tab, ..._] => (
  ...
  React.useEffect2(
    () => {
      ...
      | ["school", "courses", courseId, ..._] =>
        loadCourses(
          Some(courseId),
          state,
          None,
          send,
          ~skipSchoolStatsLoad=false,
        )
  ...
  <div className="flex min-h-full bg-gray-50">
    {switch (state.courses, editorAction) {
     ...
     | (_, ShowForm(id)) =>
       let course =
         switch (state.selectedCourse) {
         | Some(c) when Some(c.id) == id => Some(c)
         | _ =>
           Belt.Option.flatMap(id, id =>
             Some(
               ArrayUtils.unsafeFind(
                 c => Course.id(c) == id,
                 ("Unable to find course with ID: " ++ id)
                 ++ " in Courses Index",
    ...
    <div className="flex-1 flex flex-col">
      <div className="w-full">
        <div className="max-w-full mx-auto relative overflow-hidden">
          <div
            className="bg-gradient-to-r from-secondary-500 to-secondary-600 bg-cover h-40">
            {switch (School.coverImageUrl(school)) {
             ...
             | None =>
               <div
                 className="school-customize__cover-default h-full w-full svg-bg-pattern-6"
               />
             }}
          </div>
          <div className="w-full bg-white p-6">
            <div
              className="flex items-center max-w-4xl 2xl:max-w-5xl mx-auto justify-between">
           ...
           <div className="px-2 lg:px-5 mt-8">
             <div className="grid grid-cols-2 gap-x-10 gap-y-8">
               {SkeletonLoading.multiple(
                  ~count=4,
                  ~element=SkeletonLoading.imageCard(),
                )}
             </div>
           </div>
