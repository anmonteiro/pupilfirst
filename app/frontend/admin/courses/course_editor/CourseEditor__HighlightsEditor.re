...
let replace = (index, highlights, updateHighlightsCB, newHighlight) =>
  updateHighlightsCB(
    ArrayUtils.replaceWithIndex(index, newHighlight, highlights),
  );
...
let addHighlight = (highlights, updateHighlightsCB) =>
  updateHighlightsCB(
    Js.Array.concat([|Course.Highlight.empty()|], ~other=highlights),
  );

let removeHighlight = (index, highlights, updateHighlightsCB) =>
  updateHighlightsCB(
    Js.Array.filteri(~f=(_a, i) => i != index, highlights),
  );
