...
[%%raw {js|import "./CoursesReview__Root.css"|js}];
open CoursesReview__Types;
let tc = I18n.t(~scope="components.CoursesReview__Root");
module Item = {
  type nonrec t = IndexSubmission.t;
};
module PagedSubmission = Pagination.Make(Item);
type nonrec coachFilter =
  | Assigned
  | Personal
  | ReviewedBy;
type nonrec filterLoader =
  | ShowLevels
  | ShowCoaches(coachFilter)
  | ShowTargets;
type nonrec loading =
  | Unloaded
  | Loading
...
let reducer = (state, action) =>
  ...
  | 
    ...
    let updatedTopics =
      ...
      | LoadingMore =>
        Js.Array.concat(
          PagedSubmission.toArray(state.submissions),
          ~other=newTopics,
        )
      ...
      };
    {
      ...state,
      submissions:
        PagedSubmission.make(updatedTopics, hasNextPage, endCursor),
      loading: LoadingV2.setNotLoading(state.loading),
      totalEntriesCount,
      targets:
        if (ArrayUtils.isEmpty(state.targets)) {
          Belt.Option.mapWithDefault(target, [||], t => [|t|]);
        } else {
          state.targets;
        },
      levels:
        if (ArrayUtils.isEmpty(state.levels)) {
          Belt.Option.mapWithDefault(level, [||], t => [|t|]);
        } else {
          state.levels;
        },
      coaches:
        if (ArrayUtils.isEmpty(state.coaches)) {
          coaches;
        } else {
          state.coaches;
        },
  ...
  | SetLoader(loader) => {
      ...
      filterInput:
        ...
        | ShowCoaches(key) =>
          tc({js|filter_input.|js} ++ coachFilterTranslationKey(key))
...
let updateParams = filter =>
  ReasonReactRouter.push("?" ++ Filter.toQueryString(filter));
...
let getSubmissions = (send, courseId, cursor, filter) => {
  let coachIds =
    ...
    |]
    ->(Js.Array.map(~f=Belt.Option.mapWithDefault(_, [||], t => [|t|])))
    ->ArrayUtils.flattenV2;
  let variables =
    SubmissionsQuery.makeVariables(
      ~courseId,
  ...
  |> Js.Promise.then_(response => {
       let target =
         OptionUtils.map(TargetInfo.makeFromJs, response##targetInfo);
...
let getCoaches = (send, courseId, state) =>
  if (state.coachesLoaded == Unloaded) {
    ...
    |> Js.Promise.then_(response => {
         send(
           LoadCoaches(Js.Array.map(~f=Coach.makeFromJs, response##coaches)),
         );
         Js.Promise.resolve();
       })
    |> ignore;
  ...
  if (state.targetsLoaded == Unloaded) {
    ...
    |> Js.Promise.then_(response => {
         send(
           LoadTargets(
             Js.Array.map(
               ~f=TargetInfo.makeFromJs,
               response##reviewedTargetsInfo,
             ),
           ),
         );
         Js.Promise.resolve();
...
module Sortable = {
  type nonrec t = [ | `EvaluatedAt | `SubmittedAt];
  let criterion = t =>
    switch (t) {
...
module Selectable = {
  ...
  let value = t =>
    switch (t) {
    | Level(level) =>
      (string_of_int(Level.number(level)) ++ ", ") ++ Level.name(level)
    ...
    | Loader(l) =>
      ...
      | ShowCoaches(key) =>
        tc({js|coach_filter_by.|js} ++ coachFilterTranslationKey(key))
  ...
  let searchString = t =>
    ...
    | Level(level) =>
      (
        ((tc("search.level") ++ " ") ++ string_of_int(Level.number(level)))
        ++ ", "
      )
      ++ Level.name(level)
    | AssignedToCoach(coach, currentCoachId) =>
    ...
    | Loader(ShowLevels) => tc("search.level")
    | Loader(ShowCoaches(key)) =>
      tc({js|search.|js} ++ coachFilterTranslationKey(key))
    ...
    | NameOrEmail(search) => search
    | IncludeInactive =>
      (tc("include") ++ {js|: |js}) ++ tc("inactive_students")
    };
  let color = t =>
    switch (t) {
    | Level(_) => "blue"
  ...
  let assignedToCoach = (coach, currentCoachId) =>
    AssignedToCoach(coach, currentCoachId);
  let personalCoach = (coach, currentCoachId) =>
    PersonalCoach(coach, currentCoachId);
  let reviewedBy = (coach, currentCoachId) =>
    ReviewedBy(coach, currentCoachId);
...
let nameOrEmailFilter = state => {
  ...
  let firstWord = Js.String.split(input, ~sep=" ")[0];

  if ((
        (input == "" || firstWord == tc("search.level"))
...
let unselected = (state, currentCoachId, filter) => {
  let unselectedLevels =
    ...
    ->(Js.Array.map(~f=Selectable.level));
  let unselectedTargets =
    ...
    ->(Js.Array.map(~f=Selectable.target));
  let unselectedCoaches = (getCoachId, getSelectable) =>
    ...
    ->(Js.Array.map(~f=coach => getSelectable(coach, currentCoachId)));
  ...
  ArrayUtils.flattenV2([|
    ...
    unselectedReviewers,
    unselectedTargets,
    if (state.levelsLoaded == Loaded) {
      [||];
    } else {
      [|Selectable.makeLoader(ShowLevels)|];
    },
    if (state.coachesLoaded == Loaded) {
      [||];
    } else {
      [|
    ...
    if (state.targetsLoaded == Loaded) {
      [||];
    } else {
      [|Selectable.makeLoader(ShowTargets)|];
    },
    nameOrEmailFilter(state),
    if (Filter.includeInactive(filter)) {
      [||];
    } else {
...
let selected = (state, filter, currentCoachId) => {
  let selectedLevel =
    Belt.Option.mapWithDefault(Filter.levelId(filter), [||], levelId =>
      Belt.Option.mapWithDefault(
        Js.Array.find(~f=l => Level.id(l) == levelId, state.levels), [||], l =>
        [|Selectable.level(l)|]
      )
    );
  let selectedTarget =
    Belt.Option.mapWithDefault(filter.targetId, [||], targetId =>
      Belt.Option.mapWithDefault(
        Js.Array.find(~f=t => TargetInfo.id(t) == targetId, state.targets),
        [||],
        t =>
        [|Selectable.target(t)|]
      )
  ...
  let selectedCoaches = (getCoachId, getSelectable) =>
    Belt.Option.mapWithDefault(getCoachId(filter), [||], coachId =>
      Belt.Option.mapWithDefault(
        Js.Array.find(~f=c => Coach.id(c) == coachId, state.coaches), [||], c =>
        [|getSelectable(c, currentCoachId)|]
      )
    );
  let selectedAssiginedCoach =
    selectedCoaches(
      filter => filter.assignedCoachId,
      Selectable.assignedToCoach,
    );
  let selectedPersonalCoach =
    selectedCoaches(
      filter => filter.personalCoachId,
      Selectable.personalCoach,
    );
  let selectedReviewer =
    selectedCoaches(filter => filter.reviewingCoachId, Selectable.reviewedBy);
  let selectedStatus =
    OptionUtils.mapWithDefault(
...
let onSelectFilter = (send, courseId, state, filter, selectable) => {
  ...
  | Level(level) =>
    updateParams({...filter, levelId: Some(Level.id(level))})
  | Loader(l) =>
    send(SetLoader(l));
    switch (l) {
  ...
  | Target(target) =>
    updateParams({...filter, targetId: Some(TargetInfo.id(target))})
  | Status(status) =>
    switch (status) {
    | `Pending =>
      updateParams({
  ...
  | NameOrEmail(nameOrEmail) =>
    updateParams({...filter, nameOrEmail: Some(nameOrEmail)})
  | IncludeInactive => updateParams({...filter, includeInactive: true})
  };
};
let onDeselectFilter = (send, filter, selectable) =>
  switch (selectable) {
  | Selectable.AssignedToCoach(_) =>
    updateParams({...filter, assignedCoachId: None})
  ...
  | Status(_) =>
    updateParams({...filter, tab: None, sortCriterion: `SubmittedAt})
...
let submissionsLoadedData = (totalSubmissionsCount, loadedSubmissionsCount) =>
  <p
    ...
    {str(
       if (totalSubmissionsCount == loadedSubmissionsCount) {
         ...
         tc(
           ...
           ~variables=[|
             ...
             (
               ...
               string_of_int(loadedSubmissionsCount),
             ),
           |],
...
let pageTitle = (courses, courseId) => {
  let currentCourse =
    ArrayUtils.unsafeFind(
      ...
      ++ " in CoursesReview__Root",
      courses,
    );
  (tc("review") ++ {js| | |js}) ++ AppRouter__Course.name(currentCourse);
};
[@react.component]
            ...
            <div className="px-4">
              <div className="flex pt-3 md:border-b border-gray-300">
                <div
                  ...
                  <div
                    role="tab"
                    ariaSelected={filter.tab === Some(`Pending)}
                    className="flex-1">
                    <Link
                      href={
                        (("/courses/" ++ courseId) ++ "/review?")
                  ...
                  <div
                    role="tab"
                    ariaSelected={filter.tab === Some(`Reviewed)}
                    className="flex-1">
                    <Link
                      href={
                        (("/courses/" ++ courseId) ++ "/review?")
              ...
              <div className="flex-1">
                ...
                <Multiselect
                  ...
                  onChange={filterInput =>
                    send(UpdateFilterInput(filterInput))
                  }
          ...
          <div>
            {switch (state.submissions) {
             ...
             | FullyLoaded(submissions) =>
               <div> {submissionsList(submissions, state, filter)} </div>
             }}
          </div>
          {switch (state.submissions) {
           | Unloaded => React.null
