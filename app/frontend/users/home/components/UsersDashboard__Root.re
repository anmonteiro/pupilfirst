[%%raw {js|import "./UserDashboard__Root.css"|js}];
...
type nonrec view =
  ...
  | ShowCertificates;
let headerSectiom =
    (userName, preferredName, userTitle, avatarUrl, showUserEdit) => {
...
let courseLink = (href, title, icon) =>
  <a
    ...
    <i className=icon />
    <span className="font-medium ml-2"> title->str </span>
  </a>;
...
let ctaFooter = (course, currentSchoolAdmin) => {
  ...
  | `ViewCourse =>
    ctaButton(t("cta.view_course"), studentLink(courseId, "curriculum"))
  | `EditCourse =>
    ctaButton(
  ...
  | `ReviewSubmissions =>
    ctaButton(t("cta.review_submissions"), studentLink(courseId, "review"))
...
let communityLinks = (communityIds, communities) =>
  Js.Array.map(
    ~f=
      id => {
        let community =
          Js.Array.find(~f=c => Community.id(c) == id, communities);
        switch (community) {
        | Some(c) =>
          <a
            key={Community.id(c)}
           ...
           issuedCertificate =>
             <div
               ...
               <a
                 className="w-full h-full shadow rounded-lg hover:shadow-lg"
                 href={
                   "/c/" ++ issuedCertificate->IssuedCertificate.serialNumber
                 }>
                 <div
                   className="user-dashboard-community__cover flex w-full bg-gray-600 h-40 svg-bg-pattern-5 items-center justify-center p-4 shadow rounded-t-lg"
                 />
