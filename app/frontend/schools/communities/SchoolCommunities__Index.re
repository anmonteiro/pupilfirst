  ...
  | DeleteCategory(community, categoryId) =>
    let updatedCommunity = Community.removeCategory(community, categoryId);
    {
      ...
      communities:
        state.communities
        |> List.map(c =>
             if (Community.id(c) == Community.id(community)) {
               updatedCommunity;
             } else {
               c;
             }
...
let handleCloseCategoryManager = (send, state) =>
  if (categoryEditorDirty(state)) {
    if (Webapi.Dom.(window |> Window.confirm(tr("unsaved_window_confirm")))) {
              ...
              <div
                ...
                <div
                  ...
                  <button
                    ...
                    className="course-faculty__list-item-details flex flex-1 items-center justify-between border border-transparent cursor-pointer rounded-l-lg text-left hover:bg-gray-50 hover:text-primary-500 hover:border-primary-400 focus:bg-gray-50 focus:text-primary-500">
