...
type nonrec event = [ | `TopicCreated | `PostCreated];
type nonrec status = [ | `Unread | `Read];
let eventName = event =>
  switch (event) {
...
let markAllNotifications = notifications =>
  notifications
  |> Js.Array.map(~f=entry =>
       if (Entry.readAt(entry)->Belt.Option.isNone) {
         Entry.markAsRead(entry);
       } else {
         entry;
       }
...
let reducer = (state, action) =>
  ...
  | LoadNotifications(endCursor, hasNextPage, newTopics, totalEntriesCount) =>
    let updatedTopics =
      ...
      | LoadingMore =>
        Js.Array.concat(Pagination.toArray(state.entries), ~other=newTopics)
      | Reloading => newTopics
  ...
  | MarkNotification(id) => {
      ...
      totalEntriesCount:
        if (state.filter.status->Belt.Option.isSome) {
...
let getEntries = (send, cursor, filter) => {
  let variables =
    NotificationsQuery.makeVariables(
...
let unselected = state => {
  let eventFilters =
    Selectable.event->(Array.map([|`TopicCreated, `PostCreated|]));
...
let showEntries = (entries, state, send) => {
  let filteredEntries =
    ...
    ->(
        Belt.Option.mapWithDefault(entries, u =>
          ...
          | `Read =>
            Js.Array.filter(
              ~f=e => Entry.readAt(e)->Belt.Option.isSome,
              entries,
            )
          | `Unread =>
            Js.Array.filter(
              ~f=e => Entry.readAt(e)->Belt.Option.isNone,
              entries,
            )
          }
        )
  ...
  let dates =
    Js.Array.map(
      ~f=e => Js.Date.toDateString(Entry.createdAt(e)),
      filteredEntries,
    )
    ->ArrayUtils.distinct;
