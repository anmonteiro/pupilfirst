[%%raw {js|import "./StudentTopNav.css"|js}];
...
let headerLink = (key, link) =>
  <div
    key
    className="md:ml-2 text-sm font-medium text-center cursor-default flex w-1/2 sm:w-1/3 md:w-auto justify-center border-r border-b md:border-0">
    <a
...
let headerLinks = (links, isLoggedIn, user, hasNotifications) => {
  ...
  | visibleLinks =>
    visibleLinks
    |> Js.Array.mapi(~f=(l, index) => headerLink(index |> string_of_int, l))
    |> Js.Array.concat(
         ~other=[|
      ...
      <nav className="flex justify-between items-center h-20">
        <div className="flex w-full items-center justify-between">
          <a
            ...
            href={if (isLoggedIn) {"/dashboard"} else {"/"}}>
            {switch (logoUrl) {
             | Some(url) =>
               <img
          ...
          {ReactUtils.nullUnless(
             <div className="flex items-center space-x-2">
               ...
               <div onClick={_ => toggleMenuHidden(menuHidden => !menuHidden)}>
                 <div
                   className={
                     "student-navbar__menu-btn cursor-pointer hover:bg-gray-50 w-8 h-8 text-center relative focus:outline-none rounded-lg "
