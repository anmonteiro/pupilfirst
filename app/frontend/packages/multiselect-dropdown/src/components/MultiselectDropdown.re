[%%raw {js|import "./MultiselectDropdown.css"|js}];
...
type nonrec searchItem = {
  index: int,
  text: string,
};
[@mel.module "../utils/fuzzySearch"]
external fuzzySearch: (string, array(searchItem)) => array(searchItem) =
  "default";
...
module Make = (Selectable: Selectable) => {
  let search = (searchString, selections) => {
    let searchSelection =
      selections->(
                    Js.Array.mapi(~f=(s, i) =>
                      {index: i, text: Selectable.searchString(s)}
                    )
                  );
    let results =
      fuzzySearch(String.lowercase_ascii(searchString), searchSelection);
    results->(Js.Array.map(~f=searchItem => selections[searchItem.index]));
  };
  ...
  let tagPillClasses = (color, showHover) => {
    ...
    ++ (((bgColor ++ " ") ++ textColor) ++ " ")
    ++ (
      if (showHover) {"px-2 py-px hover:saturate-50 focus:saturate-50"} else {
        "inline-flex"
      }
    );
  };
      ...
      |> Array.mapi((index, selection) =>
           <button
             ...
             {switch (selection |> Selectable.label) {
              | Some(label) =>
                <span
                  className="mr-2 shrink-0 w-1/3 sm:w-auto md:w-1/3 text-right">
  ...
  let searchResult =
      (searchInput, unselected, labelSuffix, loading, id, onSelect) => {
  ...
  let showSelected = (onDeselect, labelSuffix, selected) =>
    ...
    |> Array.mapi((index, selection) => {
         ...
         <div
           key={index |> string_of_int}
           className={
             tagPillClasses(selection |> Selectable.color, false)
             ++ " flex gap-px"
           }>
      ...
      <div>
        <div
          ...
          <input
            ...
            autoComplete="off"
            value
            onChange={e => onChange(React.Event.Form.target(e)##value)}
      ...
      {switch (showDropdown, results, defaultOptions, hint) {
       ...
       | (true, [||], [||], None) =>
         if (value == "") {
           ...
           wrapper(str(emptyMessage));
         }
       | (true, [||], [||], Some(hint)) => wrapper(showHint(hint))
       | (true, [||], options, None) =>
         wrapper(
           React.array(
             showOptions(options, onSelect, inputId, labelSuffix, loading),
           ),
         )
