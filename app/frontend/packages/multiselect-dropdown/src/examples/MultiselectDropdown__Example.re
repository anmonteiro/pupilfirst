...
module DetailedExample = {
  ...
  let deselect = (selected, setState, selectable) => {
    let newSelected = selected |> Js.Array.filter(~f=s => s != selectable);
...
| Some(element) =>
  ...
  ReactDOM.Client.render(root, <DetailedExample />);
| None => ()
};
switch (ReactDOM.querySelector("#MinimalExample")) {
| Some(element) =>
  let root = ReactDOM.Client.createRoot(element);
  ReactDOM.Client.render(root, <MinimalExample />);
| None => ()
};
