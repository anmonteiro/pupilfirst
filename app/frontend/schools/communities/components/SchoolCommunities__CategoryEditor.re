...
let makeDeleteCategoryQuery = (categoryId, deleteCategoryCB, send) => {
  ...
  |> Js.Promise.then_((response: DeleteCategoryQuery.t) => {
       if (response.deleteTopicCategory.success) {
  ...
  |> Js.Promise.catch(error => {
       ...
       Notification.error(
         ...
         ts("notifications.please_reload"),
       );
       Js.Promise.resolve();
     })
  |> ignore;
...
let deleteCategory = (category, deleteCategoryCB, send, event) => {
  ...
  if (topicsCount > 0) {
    if (Webapi.Dom.(window |> Window.confirm(tr("topics_delete_confirm")))) {
...
let updateCategory = (category, newName, updateCategoryCB, send, event) => {
  ...
  send(StartSaving);
  let variables =
    UpdateCategoryQuery.makeVariables(
  ...
  |> Js.Promise.then_((response: UpdateCategoryQuery.t) => {
       if (response.updateTopicCategory.success) {
...
let make =
  ...
  | Some(category) =>
    ...
    <div
      ...
      <div>
        {if (presentCategoryName == state.categoryName) {
           <span
