...
let updateCourse = (state, send, updateApplicantCB, applicant) => {
  ...
  let variables =
    CreateStudentFromApplicant.makeVariables(
...
let reducer = (state, action) =>
  ...
  | AddTag(tag) => {
      ...state,
      tagsToApply: Js.Array.concat([|tag|], ~other=state.tagsToApply),
    }
  | RemoveTag(tag) => {
      ...state,
      tagsToApply: Js.Array.filter(~f=t => t != tag, state.tagsToApply),
    }
...
let optionalText = () =>
  <span className="text-xs ml-1">
    {(("(" ++ I18n.ts("optional")) ++ ")")->str}
  </span>;

let showActionsTab =
    (state, send, applicant: Applicant.t, tags, updateApplicantCB) =>
  <div>
    <div className="pt-6">
    ...
    <School__SearchableTagList
      unselectedTags={Js.Array.filter(
        ~f=tag => !(state.tagsToApply |> Array.mem(tag)),
        tags,
      )}
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="w-full pt-6">
          <div
            ...
            <button
              className={
                "-ml-px " ++ selectedTabClasses(selectedTab == ActionsTab)
              }
              onClick={_ =>
                ReasonReactRouter.push(
          ...
          <Spread props={"applicant-id": Applicant.id(applicant)}>
            {switch (selectedTab) {
             ...
             | ActionsTab =>
               showActionsTab(state, send, applicant, tags, updateApplicantCB)
             }}
          </Spread>
        </div>
      </div>
