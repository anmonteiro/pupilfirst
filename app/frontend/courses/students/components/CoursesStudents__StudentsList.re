...
let levelInfo = level =>
  <span
    ...
    <p className="font-bold">
      {level->Shared__Level.number |> string_of_int |> str}
    </p>
  </span>;
...
let studentTags = student =>
  if (StudentInfo.taggings(student) |> ArrayUtils.isNotEmpty) {
    <div className="hidden md:flex flex-wrap">
...
let make = (~students) =>
  <div>
    {if (ArrayUtils.isEmpty(students)) {
       ...
       students->(Js.Array.map(~f=showStudent))->React.array;
     }}
  </div>;
