...
[@mel.module "./webPushSubscription"]
...
let handleNotificationBlock = () =>
  Notification.error(
    ...
    t("notification_rejected_message"),
  );
  ...
  | SubscribedOnAnotherDevice =>
    ...
    |> Js.Promise.then_(r => {
         ...
         | (Some(endpoint1), Some(subscription)) =>
           send(
             if (endpoint1 == subscription.endpoint) {
               SetStatusSubscribed;
             } else {
               SetStatusSubscribedOnAnotherDevice;
             },
...
let computeInitialState = () => {
  status:
    webPushEndpoint->(
                       Belt.Option.mapWithDefault(UnSubscribed, _u =>
                         Subscribed
                       )
                     ),
  saving: false,
...
let button = (saving, onClick, icon, text) =>
  <button
    ...
    <FaIcon
      classes={
        "mr-2 fa-fw fas fa-" ++ (if (saving) {"spinner fa-spin"} else {icon})
      }
    />
    {str(text)}
  </button>;
...
let make = () => {
  ...
  | UnSubscribed =>
    button(state.saving, createSubscription(send), "bell", t("subscribe"))
  | SubscribedOnAnotherDevice =>
    <div>
      {button(
         state.saving,
         ...
         "bell",
         t("subscribed_on_another_device"),
       )}
    </div>
  };
};
