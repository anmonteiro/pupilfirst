...
type nonrec markdown = string;
type nonrec url = string;
type nonrec title = string;
type nonrec caption = string;
type nonrec embedCode = option(string);
type nonrec filename = string;
type nonrec requestSource = [ | `User | `VimeoUpload];
type nonrec lastResolvedAt = option(Js.Date.t);
type nonrec width =
  | Auto
  | Full
  | FourFifths
...
let makeEmbedBlock = (url, embedCode, requestSource, lastResolvedAt) =>
  Embed(url, embedCode, requestSource, lastResolvedAt);
let makeAudioBlock = (fileUrl, title, fileName) =>
  Audio(fileUrl, title, fileName);
