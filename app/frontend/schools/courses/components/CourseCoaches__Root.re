...
let handleResponseCB = (send, json) => {
  ...
  Notification.success(
    ...
    t("enrollment_delete_notificaion_success_body"),
  );
};
...
let make = (~courseCoaches, ~schoolCoaches, ~courseId, ~authenticityToken) => {
  ...
  <DisablingCover
    containerClasses="min-h-full bg-gray-50 w-full" disabled={state.saving}>
    <div
      key="School admin coaches course index"
      className="flex min-h-full bg-gray-50">
      ...
      <div className="flex-1 flex flex-col">
        {if (Array.length(schoolCoaches) == Array.length(state.courseCoaches)
        ...
        {if (state.courseCoaches |> ArrayUtils.isEmpty) {
           <div
