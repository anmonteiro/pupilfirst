...
let selected = currentCourse => {
  let title =
    currentCourse->(
                     Belt.Option.mapWithDefault("Select Course", c =>
                       Course.name(c)
                     )
                   );
  <button
    ...
    <span className="w-5/6 flex items-center">
      ...
      <span className="block whitespace-nowrap px-2 text-left">
        title->str
      </span>
    </span>
  ...
  Js.Array.map(
    ...
    Js.Array.filter(
      ~f=
        course =>
          currentCourse->(
                           Belt.Option.mapWithDefault(true, c =>
                             Course.id(course) != Course.id(c)
                           )
                         )
          && !Course.ended(course),
      courses,
