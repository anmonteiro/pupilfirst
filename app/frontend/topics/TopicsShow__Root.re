[%%raw {js|import "./TopicsShow__Root.css"|js}];
...
let reducer = (state, action) =>
  ...
  | SaveReply(newReply, replyToPostId) =>
    ...
    | Some(id) =>
        ...
        state.replies |> Post.find(id) |> Post.addReply(newReply |> Post.id);
      {
        ...
        replies:
          state.replies
          |> Js.Array.filter(~f=r => Post.id(r) != id)
  ...
  | LikeReply(post) =>
    let updatedPost = post |> Post.addLike;
    {
      ...
      replies:
        state.replies
        |> Js.Array.filter(~f=reply => Post.id(reply) != Post.id(post))
  ...
  | UpdateReply(reply) => {
      ...
      replies:
        state.replies
        |> Js.Array.filter(~f=r => Post.id(r) != Post.id(reply))
        |> Array.append([|reply|]),
    }
  | ArchivePost(postId) => {
      ...state,
      replies: state.replies |> Js.Array.filter(~f=r => Post.id(r) != postId),
    }
...
let subscribe = (send, ()) => send(Subscribe);
let unsubscribe = (send, ()) => send(Unsubscribe);
...
let updateTopic = (state, send, event) => {
  ...
  let variables =
    UpdateTopicQuery.makeVariables(
...
let topicSolutionId = replies =>
  Js.Array.find(~f=Post.solution, replies)->(Belt.Option.map(Post.id));
[@react.component]
let make =
    (
      ~topic,
    ...
    <div className="flex-col items-center justify-between">
      ...
      <div
          ...
          {if (state.showTopicEditor) {
             ...
             <div className="flex flex-col ">
               <div
                 ...
                 <span className="flex">
                   {if (moderator || isTopicCreator(firstPost, currentUserId)) {
                      <button
        ...
        <h5 className="pt-4 pb-2 lg:ml-14 border-b">
          {(Belt.Int.toString(Array.length(state.replies)) ++ " ")
           ++ ts("replies")
           |> str}
        </h5>
        {state.replies
        ...
        {switch (Topic.lockedAt(state.topic)) {
         | Some(_lockedAt) =>
           <div
             ...
             <span className="ml-2">
               {t("locked_topic_notice")->React.string}
             </span>
           </div>
         | None =>
