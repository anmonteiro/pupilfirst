...
type nonrec targetStatus = [ | `PendingReview | `Rejected | `Completed];
...
module Selectable = {
  ...
  let label = t =>
    switch (t) {
    | Level(level) =>
      Some(LevelLabel.format(level |> Level.number |> string_of_int))
...
let unselected = (levels, selectedLevel, selectedStatus) => {
  let unselectedLevels =
    levels
    |> Js.Array.filter(~f=level => Level.number(level) != 0)
...
let updateStudentSubmissions =
  ...
  let updatedSubmissions =
    Js.Array.concat(Submission.makeFromJs(nodes), ~other=submissions);
     ...
     |> Array.map(submission => {
          ...
          <div className="" key={submission |> Submission.id}>
            <a
              ...
              ariaLabel={
                "Student submission " ++ (submission |> Submission.id)
              }
              href=submissionHref>
              <div
                ...
                <div className="w-full md:w-3/4">
                  ...
                  <div className="mt-1 ml-px text-xs text-gray-900">
                    <span className="ml-1">
                      {tr(
                         ~variables=[|
                           ("date", submission |> Submission.createdAtPretty),
                         |],
                         "submitted_on",
                       )
                       |> str}
                     ...
                     <HelpIcon className="ml-1">
                       <span
                         dangerouslySetInnerHTML=[%obj
                           {__html: tr("submission_not_considered_help")}
                         ]
                       />
                     </HelpIcon>
                   </div>
  ...
  <div className="max-w-3xl mx-auto">
    <div role="form" className="md:flex items-end w-full pb-4">
      <div className="flex-1">
        <label
          htmlFor="filter" className="block text-tiny font-semibold uppercase">
          {"Filter by:" |> str}
        </label>
        <Multiselect
          id="filter"
    ...
    <div ariaLabel="Student submissions">
      {switch ((submissions: Submissions.t)) {
       ...
       | PartiallyLoaded({submissions, _}, cursor) =>
         <div>
