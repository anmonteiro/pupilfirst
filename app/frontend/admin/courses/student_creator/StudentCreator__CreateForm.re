...
let allKnownTags = (incomingTags, appliedTags) =>
  incomingTags |> Js.Array.concat(~other=appliedTags) |> ArrayUtils.distinct;
let handleResponseCB = (state, studentIds) => {
  ...
  if (studentsAdded == 0) {
    ...
    let studentsRequested =
      Js.Array.reduce(
        ~f=(acc, team) => acc + TeamInfo.students(team)->Js.Array.length,
        ~init=0,
        state.teamsToAdd,
      );
...
let loadData = (courseId, send) => {
  send(SetLoading);
  StudentsCreateDataQuery.fetch(
    StudentsCreateDataQuery.makeVariables(~courseId, ()),
  )
...
let createStudents = (state, send, courseId, cohort, event) => {
  ...
  | {notifyStudents, _} =>
    let variables =
      CreateStudentsQuery.makeVariables(
...
let teamHeader = (teamName, studentsCount) =>
  <div className="flex justify-between mb-1">
    <span className="text-tiny font-semibold">
      <span>
        {t(~variables=[|("team_name", teamName)|], "team_header_label")->str}
      </span>
    </span>
    {if (studentsCount < 2) {
    ...
    <div className="flex flex-col flex-1 flex-wrap p-3">
      <div className="flex items-center">
        <div className="mr-1 font-semibold">
          {StudentInfo.name(studentInfo)->str}
        </div>
    ...
    <div className="pt-6">
      ...
      <div className="p-4 rounded-lg border bordedr-gray-300 bg-gray-100 mt-1">
                 ...
                 Js.Array.map(~f=team =>
                   ...
                   | 
                     ...
                     <div className="mt-3" key=teamName>
                       ...
                       <div
                         className="bg-white border shadow rounded-lg mt-2 px-2">
                         {studentsInTeam
                          ->(
                              Js.Array.map(~f=studentInfo =>
        ...
        <div className="mt-4 flex">
          ...
          />
          <label
            className="checkbox-label flex " htmlFor="notify-new-students">
        ...
        <div className="flex mt-4">
          {switch (state.selectedCohort) {
           ...
           | None =>
             <button
               disabled=true
               className="w-full btn btn-primary btn-large mt-3 disabled">
               {t("select_cohort")->str}
             </button>
           }}
        </div>
