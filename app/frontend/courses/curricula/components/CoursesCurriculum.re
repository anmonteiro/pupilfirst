[%%mel.raw {js|import "./CoursesCurriculum.css"|js}];

[@mel.module "../images/level-lock.svg"]
external levelLockedImage: string = "default";
[@mel.module "../images/level-empty.svg"]
  ...
  <div
    ...
    <Link
      ...
      {ReactUtils.nullIf(
         ...
         TargetStatus.isAccessEnded(targetStatus)
    ...
    {ReactUtils.nullUnless(
       <a
         title={t(
           "edit_target_button_title",
           ~variables=[|("title", Target.title(target))|],
         )}
         ariaLabel={t(
           "edit_target_button_title",
           ~variables=[|("title", Target.title(target))|],
         ...
         href={
           ((("/school/courses/" ++ courseId) ++ "/targets/") ++ targetId)
           ++ "/content"
         }
...
let renderTargetGroup =
  ...
  <div
    ...
    <div
      ...
      {if (targetGroup |> TargetGroup.milestone) {
         <div
      ...
      {targets
       |> ArrayUtils.copyAndSort((t1, t2) =>
            (t1 |> Target.sortIndex) - (t2 |> Target.sortIndex)
          )
       |> Js.Array.map(~f=target =>
            rendertarget(target, statusOfTargets, author, courseId)
          )
       |> React.array}
    </div>
  </div>;
  ...
  setState(state => {
...

    {
      ...
      latestSubmissions:
        Js.Array.concat(
          [|latestSubmission|],
          ~other=withoutSubmissionForThisTarget,
        ),
...
let statusOfMilestoneTargets = (targetGroups, targets, level, statusOfTargets) => {
  ...
  let milestoneTargetGroupIds =
    ...
    |> Js.Array.map(~f=tg => tg |> TargetGroup.id);

  let milestoneTargetIds =
    targets
    |> Js.Array.filter(~f=t =>
         milestoneTargetGroupIds
         |> Js.Array.includes(~value=Target.targetGroupId(t))
       )
    |> Js.Array.map(~f=t => t |> Target.id);
...
let computeLevelUp =
  ...
  | AtMaxLevel =>
    if (TargetStatus.allComplete(statusOfCurrentMilestoneTargets)) {
      CourseComplete;
    } else {
      Nothing;
    }
...
let navigationLink = (direction, level, setState) => {
  ...
  let arrow = icon =>
    icon->(
            Belt.Option.mapWithDefault(React.null, icon =>
...
let make =
  ...
  let levelZero = levels |> Js.Array.find(~f=l => l |> Level.number == 0);
      ...
      let statusOfTargets = computeTargetStatus(submissions);
      {
        selectedLevelId:
          ...
          | (_, Some(targetLevelId), Some(levelZero)) =>
            if (levelZero |> Level.id == targetLevelId) {
              studentLevelId;
            } else {
              targetLevelId;
            }
  ...
  let targetGroupsInLevel =
    targetGroups
    |> Js.Array.filter(~f=tg => tg |> TargetGroup.levelId == currentLevelId);

  <div
    role="main" ariaLabel="Curriculum" className="bg-gray-50 pt-11 pb-8 -mt-7">
    {switch (selectedTarget) {
     | Some(target) =>
       let targetStatus =
     ...
     | _anyOtherNotice =>
       [|
         <div className="relative" key="curriculum-body">
           ...
           {ReactUtils.nullUnless(
              <div className="text-center mt-2 max-w-3xl mx-auto">
                <a
                  ...
                  href={
                    (
                      ("/school/courses/" ++ Course.id(course))
                      ++ "/curriculum?level="
                    )
                  ...
                  <i className="fas fa-pencil-alt" />
                  <span className="ml-2">
                    {t("edit_level_button")->str}
                  </span>
                </a>
              </div>,
              author,
            )}
           {if (currentLevel |> Level.isLocked && accessLockedLevels) {
              <div
                ...
                dangerouslySetInnerHTML=[%obj
                  {
                    __html:
                      t(
                        ...
                        ~variables=[|
                          ("date", Level.unlockDateString(currentLevel)),
                        |],
                      ),
                  }
                ]
           ...
           {if (Level.isUnlocked(currentLevel) || accessLockedLevels) {
              if (targetGroupsInLevel == [||]) {
