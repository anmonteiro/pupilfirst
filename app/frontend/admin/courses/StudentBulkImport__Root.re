...
let ts = I18n.ts;
module CSVData = {
  type nonrec t = StudentCSVData.t;
};
module CSVReader = CSVReader.Make(CSVData);
type nonrec fileInvalid =
  | InvalidCSVFile
  | EmptyFile
  | InvalidTemplate
  | ExceededEntries
...
let validateFile = (csvData, fileInfo) =>
  if (CSVReader.fileSize(fileInfo) > FileUtils.maxUploadFileSize) {
    Some(InvalidCSVFile);
  } else if (ArrayUtils.isEmpty(csvData)) {
    Some(EmptyFile);
  } else if (!validTemplate(csvData)) {
    Some(InvalidTemplate);
  } else if (Array.length(csvData) > 1000) {
    Some(ExceededEntries);
  } else {
    let dataErrors = CSVDataError.parseError(csvData);

    if (ArrayUtils.isNotEmpty(dataErrors)) {
      Some(InvalidData(dataErrors));
    } else {
      None;
    };
  };
...
let fileInputText = (~fileInfo: option(CSVReader.fileInfo)) =>
  fileInfo->(
              Belt.Option.mapWithDefault(t("csv_file_input.placeholder"), info =>
                info.name
              )
            );
...
let submitForm = (cohort, exitUrl, send, event) => {
  ...
  | Some(c) =>
    ...
    Api.sendFormData(
      ...
      formData,
      json => {
        if (Json.Decode.field("success", Json.Decode.bool, json)) {
          Notification.success(
            ts("notifications.done_exclamation"),
            t("success_notification"),
          );
          ReasonReactRouter.push(exitUrl);
        } else {
          ();
        };
        send(EndSaving);
...
let tableHeader =
  <thead>
    <tr className="bg-gray-300">
      <th
        className="w-12 border border-gray-400 text-left text-xs px-2 py-1 font-semibold"
      />
      <th
        className="border border-gray-400 text-left text-xs px-2 py-1 font-semibold">
        "name"->str
      </th>
      <th
        className="border border-gray-400 text-left text-xs px-2 py-1 font-semibold">
    ...
    ~f=
      (studentData, index) =>
        <tr key={string_of_int(index)}>
          <td
            className="w-12 bg-gray-300 border border-gray-400 truncate text-xs px-2 py-1">
            {string_of_int(startingRow + index + 2)->str}
          </td>
          <td className="border border-gray-400 truncate text-xs px-2 py-1">
            {StudentCSVData.name(studentData)
             ->(Belt.Option.getWithDefault(""))
             ->str}
          </td>
          <td className="border border-gray-400 truncate text-xs px-2 py-1">
            {StudentCSVData.email(studentData)
             ->(Belt.Option.getWithDefault(""))
             ->str}
          </td>
          <td className="border border-gray-400 truncate text-xs px-2 py-1">
            {StudentCSVData.title(studentData)
             ->(Belt.Option.getWithDefault(""))
             ->str}
          </td>
          <td className="border border-gray-400 truncate text-xs px-2 py-1">
            {StudentCSVData.teamName(studentData)
             ->(Belt.Option.getWithDefault(""))
             ->str}
          </td>
          <td className="border border-gray-400 truncate text-xs px-2 py-1">
            {StudentCSVData.tags(studentData)
             ->(Belt.Option.getWithDefault(""))
             ->str}
          </td>
          <td className="border border-gray-400 truncate text-xs px-2 py-1">
            {StudentCSVData.affiliation(studentData)
             ->(Belt.Option.getWithDefault(""))
             ->str}
          </td>
        </tr>,
...
let truncatedTable = csvData => {
  ...
  let lastTwoRows =
    Js.Array.slice(~start=Js.Array.length(csvData) - 2, csvData);
  <div>
      ...
      <tbody>
        <tr
          ...
          <td colSpan=3 className="px-2 py-3">
            <div>
              <div
                className="absolute inset-0 flex items-center" ariaHidden=true>
                <div
                  className="w-full border-t border-b py-1 border-dashed border-gray-400"
                />
              </div>
              <div className="relative flex justify-center">
                <span
                  className="px-2 bg-white text-xs italic text-center text-gray-700">
                  {(
                     ("- - - " ++ string_of_int(Array.length(csvData) - 4))
                     ++ " Rows - - -"
                   )
                   ->str}
                </span>
    ...
    <table className="table-fixed border w-full overflow-x-scroll">
      <tbody>
        {tableRows(
           lastTwoRows,
           ~startingRow=Js.Array.length(csvData) - 2,
           (),
         )}
      </tbody>
    </table>
  ...
  ReactUtils.nullIf(
    <div className="mt-4">
      ...
      <p className="font-semibold text-xs mt-4">
        {t("valid_data_summary_text")->str}
      </p>
      {if (Js.Array.length(csvData) <= 10) {
         <table className="table-fixed mt-2 border w-full overflow-x-scroll">
           tableHeader
...
let rowClasses = hasError =>
  ...
  ++ (if (hasError) {"bg-red-200 text-red-800"} else {""});
      ...
      {Js.Array.mapi(
         ~f=
           (error, index) => {
             ...
             let studentData = Js.Array.unsafe_get(csvData, rowNumber - 2);
             <tr key={string_of_int(index)}>
               <td className={rowClasses(false)}>
               ...
               <td
                 className={rowClasses(CSVDataError.hasTeamNameError(error))}>
                 {StudentCSVData.teamName(studentData)
                  ->(Belt.Option.getWithDefault(""))
                  ->str}
               </td>
               ...
               <td
                 className={rowClasses(
                   CSVDataError.hasAffiliationError(error),
                 )}>
                 {StudentCSVData.affiliation(studentData)
                  ->(Belt.Option.getWithDefault(""))
                  ->str}
...
let errorTabulation = (csvData, fileInvalid) =>
  ...
  | Some(fileInvalid) =>
    ...
    | InvalidData(errors) =>
      <div>
        {if (errors->Js.Array.length > 10) {
           <div>
             {errorsTable(
                csvData,
                Js.Array.slice(~start=0, ~end_=10, errors),
              )}
             <div className="text-red-700 text-sm mt-5">
               ...
               <textArea
                 ...
                 {Js.Array.map(
                    ~f=error => CSVDataError.rowNumber(error),
                    Js.Array.slice(errors, ~start=10),
                  )
                  ->(Js.Array.join(~sep=","))
                  ->str}
               </textArea>
        ...
        <div className="text-red-700 text-sm mt-4 rounded-md bg-red-100 p-3">
          <div className="text-sm font-semibold pb-2">
            {t("error_summary_title")->str}
          </div>
          <ul className="list-disc list-inside text-xs">
            {errors
             |> Js.Array.map(~f=error => CSVDataError.errors(error))
             |> ArrayUtils.flattenV2
             |> ArrayUtils.distinct
             |> Js.Array.mapi(~f=(error, index) =>
                  <li key={string_of_int(index)}>
                    {str(
                       switch (error) {
                       | CSVDataError.Name =>
                         t("csv_data_errors.invalid_name")
                       ...
                       | Affiliation =>
                         t("csv_data_errors.invalid_affiliation")
...
let make = (~courseId) => {
  ...
  let exitUrl = ({js|/school/courses/|js} ++ courseId) ++ {js|/students|js};
  <div>
    <School__PageHeader
      exitUrl
      title={t("page_title")}
    ...
    <div className="max-w-4xl 2xl:max-w-5xl mx-auto px-4">
      ...
      <form onSubmit={submitForm(state.selectedCohort, exitUrl, send)}>
        ...
        <input
          ...
          type_="hidden"
          value={string_of_bool(state.notifyStudents)}
        />
        <div className="mx-auto">
          <div className="py-6 mx-auto">
            <DisablingCover
              disabled={state.saving} message={ts("processing") ++ "..."}>
              <div className="mt-5">
                <div className="flex justify-between items-center text-center">
                  <div>
                    ...
                    <HelpIcon
                      className="ml-2" link={t("csv_file_input.help_url")}>
                ...
                <div
                  ...
                  <CSVReader
                    ...
                    onFileLoaded={(x, y) => send(LoadCSVData(x, y))}
                    onError={_ =>
                      send(UpdateFileInvalid(Some(InvalidCSVFile)))
                    }
                  />
                  <label
                ...
                <School__InputGroupError
                  message={
                    ...
                    | Some(invalidStatus) =>
                      ...
                      | InvalidTemplate =>
                        t("csv_file_errors.invalid_template")

                      | ExceededEntries =>
                        t("csv_file_errors.exceeded_entries")
