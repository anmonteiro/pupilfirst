[%%raw {js|import "./AppRouter__Nav.css"|js}];
...
let showLink = (id, selectedPage, page, classes, title, contents) =>
  if (Page.useSPA(selectedPage, Page.SelectedCourse(id, page))) {
    <Link key=title href={Page.coursePath(id, page)} className=classes title>
      contents
    </Link>;
  } else {
    <a key=title href={Page.coursePath(id, page)} className=classes title>
      contents
    </a>;
  };
         ...
         ->(
             Js.Array.map(~f=link => {
               ...
               let classes =
                 ...
                 ++ (
                   if (link == coursePage) {
                     "app-router-navbar__primary-nav-link--active";
                   } else {
                     "";
                   }
                 );
               showLink(
                 ...
                 [|
                   <Icon
                     key="icon"
                     className={
                       ({js|if |js} ++ icon) ++ {js| text-xl if-fw|js}
                     }
                   />,
           ...
           Js.Array.map(~f=link => {
             ...
             showLink(
               ...
               [|
                 <Icon
                   key="icon"
                   className={({js|if |js} ++ icon) ++ {js| text-lg if-fw|js}}
                 />,
    ...
    <div className="flex shrink-0" key="main">
      <div className="flex flex-1 flex-col">
        ...
        <div className="md:hidden">
          {courseSelector(courses, selectedPage)}
        </div>
      </div>
