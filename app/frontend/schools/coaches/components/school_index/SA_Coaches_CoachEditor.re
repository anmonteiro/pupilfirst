...
let reducer = (state, action) =>
  ...
  | UpdateExited(exited) => {...state, exited, dirty: true}
...
let updateEmail = (send, email) =>
  send(UpdateEmail(email, email |> emailInvalid));
...
let make = (~coach, ~closeFormCB, ~updateCoachCB, ~authenticityToken) => {
  let (state, send) =
    React.useReducerWithMapState(reducer, coach, computeInitialState);
  ...
  let addCoach = json => {
    ...
    switch (coach) {
    | Some(_) =>
      Notification.success(ts("notifications.success"), t("coach_updated"))
    | None =>
      Notification.success(ts("notifications.success"), t("coach_created"))
    };
                  ...
                  <div
                    ...
                    <input
                      ...
                      onChange={event =>
                        send(
                          UpdateImageFileName(
                            React.Event.Form.target(event)##files[0]##name,
                          ),
                        )
                      }
                    />
