...
[%%raw {js|import "./CoursesCurriculum__Overlay.css"|js}];
...
let selectableTabs = targetDetails =>
  if (TargetDetails.communities(targetDetails) == [||]) {
    [|Learn|];
  } else {
    [|Learn, Discuss|];
  };
...
let tabLink = (tab, state, send, targetStatus) =>
  <button
    onClick={handleTablink(send)} className={tabClasses(tab, state.tab)}>
    {tab |> tabToString(targetStatus) |> str}
  </button>;
  ...
  <div
    ...
    {switch (targetStatus |> TargetStatus.status, completionType) {
     ...
     | (Locked(CourseLocked | AccessLocked), Evaluated | TakeQuiz) =>
       if (TargetDetails.submissions(targetDetails) != [||]) {
         tabButton(Complete(completionType), state, send, targetStatus);
       } else {
         React.null;
       }
...
let overlayStatus = (course, target, targetStatus, preview) =>
  <div>
    ...
    {ReactUtils.nullUnless(
       <div> {renderLocked(t("preview_mode_text"))} </div>,
       preview,
     )}
  </div>;
let renderLockReason = reason =>
  TargetStatus.lockReasonToString(reason)->renderLocked;
let prerequisitesIncomplete = (reason, target, targets, statusOfTargets, send) => {
  ...
  let prerequisiteTargets =
    ...
    |> Js.Array.filter(~f=target =>
         prerequisiteTargetIds |> Js.Array.includes(~value=Target.id(target))
       );
       ...
       |> Js.Array.map(~f=target => {
            let targetStatus =
              ...
              |> ArrayUtils.unsafeFind(
                   ...
                   "Could not find status of target with ID "
                   ++ Target.id(target),
                 );
            <Link
...
let handleLocked = (target, targets, targetStatus, statusOfTargets, send) =>
  ...
  | Locked(reason) =>
    ...
    | CourseLocked
    | AccessLocked
    | LevelLocked(_) => renderLockReason(reason)
    }
  | Pending
  | PendingReview
  | Completed
...
let overlayContentClasses = bool => if (bool) {""} else {"hidden"};
let learnSection =
    (
      send,
      targetDetails,
  ...
  let suffixLinkInfo =
    ...
    | (Pending | Rejected, TargetDetails.Evaluated) =>
      Some((
        Complete(completionType),
        t("learn_cta_submit_work"),
        "fas fa-feather-alt",
      ))
    | (Pending | Rejected, TakeQuiz) =>
      Some((
        Complete(completionType),
        t("learn_cta_take_quiz"),
        "fas fa-tasks",
      ))
  ...
  let linkToTab =
    Belt.Option.mapWithDefault(
      suffixLinkInfo, React.null, ((tab, linkText, iconClasses)) =>
      <button
        ...
        <span>
          <FaIcon classes={iconClasses ++ " mr-2"} />
          {str(linkText)}
        </span>
      </button>
    );
...
let completeSection =
  ...
  <div className={completeSectionClasses(state.tab, completionType)}>
    {switch (targetStatus |> TargetStatus.status, completionType) {
     | (Pending, TargetDetails.Evaluated) =>
       [|
...
let renderPendingStudents = (pendingUserIds, users) =>
  <div className="max-w-3xl mx-auto text-center mt-4">
    <div className="font-semibold text-md">
      {t("pending_team_members_notice")->str}
    </div>
    <div className="flex justify-center flex-wrap">
      {pendingUserIds
       |> Js.Array.map(~f=studentId => {
            let user =
              users
              |> ArrayUtils.unsafeFind(
  ...
  <div className="pb-6">
    ...
    <div
      ...
      <div className="w-1/3 mr-2">
        previous->(
                    Belt.Option.mapWithDefault(React.null, previousUrl =>
      ...
      <div className="w-1/3 ml-2">
        next->(
                Belt.Option.mapWithDefault(React.null, nextUrl =>
...
let make =
  ...
  <div
    ...
    {switch (state.targetDetails) {
     | Some(targetDetails) =>
       let completionType =
         targetDetails |> TargetDetails.computeCompletionType;
       <div>
         <div
