...
let reducer = (state, action) =>
  ...
  | LoadTeams(endCursor, hasNextPage, teams, totalEntriesCount) =>
    let updatedTeams =
      ...
      | LoadingMore =>
        Js.Array.concat(PagedTeams.toArray(state.teams), ~other=teams)
      | Reloading(_) => teams
      };
...
let studentCard = student =>
  <div
    className="flex gap-4 items-center p-4 rounded-lg bg-white border border-gray-200 ">
    <div>
      <Avatar
        name={UserProxy.name(student)}
        className="w-10 h-10 rounded-full"
      />
    </div>
    <div>
      <p className="text-sm font-semibold">
        {UserProxy.name(student)->str}
      </p>
  ...
  |> Js.Promise.then_((response: CourseTeamsQuery.t) => {
       send(
         LoadTeams(
           ...
           ->(
               Js.Array.map(~f=(t: CourseTeamsQuery.t_teams_nodes) =>
                 Team.make(
                   ~id=t.id,
                   ~name=t.name,
                   ~students=
     ...
     ->(
         Js.Array.map(~f=team =>
           <Spread props={"data-team-name": Team.name(team)}>
             <div
               className="teams-container p-6 bg-white rounded-lg shadow"
               key={Team.id(team)}>
               <div className="flex items-center justify-between">
                 ...
                 <Link
                   href={
                     ({js|/school/teams/|js} ++ Team.id(team))
                     ++ {js|/details|js}
                   }
      ...
      <div className="max-w-4xl 2xl:max-w-5xl mx-auto px-4">
        <div className="flex justify-between items-end gap-2">
          ...
          <Link
            ...
            href={({js|/school/courses/|js} ++ courseId) ++ {js|/teams/new|js}}>
