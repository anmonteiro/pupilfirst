...
let t =
  I18n.t(~scope="components.CurriculumEditor__TargetChecklistItemEditor");
let ts = I18n.ts;
...
let updateOptional = (checklistItem, updateChecklistItemCB, event) => {
  let optional = React.Event.Form.target(event)##checked;
...
let checklistDropdown = (checklistItem, updateChecklistItemCB) => {
  ...
  let selected =
    <button
      className={
        "border focus:outline-none appearance-none flex items-center rounded focus:ring focus:ring-focusColor-500 "
        ++ selectedButtonColor
      }>
      <div className="flex">
        <span
  ...
  let contents =
    kindTypes
    |> Js.Array.filter(~f=kind => kind != selectedKind)
    |> Js.Array.mapi(~f=(kind, index) =>
         <button
...
let updateChoiceText =
  ...
  let choice = React.Event.Form.target(event)##value;
  let newChecklistItem =
    checklistItem
...
let isRequiredStepTitleDuplicated = (checklist, item) => {
  ...
  if (trimmedTitle == "") {
    ...
    checklist
    |> Js.Array.filter(~f=item => !ChecklistItem.optional(item))
  ...
  <div
    ...
    <div className="w-full bg-gray-50 border rounded-lg p-5 mr-1">
      ...
      <div className="py-2 mt-2 ">
        <MarkdownEditor
          textareaId={
            "checklist-item-" ++ string_of_int(index + 1) ++ "-title"
          }
    ...
    <div
      ariaLabel={
        (t("controls_checklist") ++ " ") ++ (index + 1 |> string_of_int)
      }
