...
let searchForSimilarTopics = (send, title, communityId, ()) => {
  ...
  |> Js.Promise.then_(result => {
       let suggestions =
         result##similarTopics->(Js.Array.map(~f=TopicSuggestion.makeFromJs));
  ...
  if (title |> isInvalidString || trimmedTitle == state.similar.search) {
    ...
    let timeoutId =
      Js.Global.setTimeout(
        ~f=searchForSimilarTopics(send, trimmedTitle, communityId),
        1500,
      );
...
let suggestions = state => {
  ...
  if (suggestions |> ArrayUtils.isNotEmpty) {
    <div className="pt-3">
      ...
      {suggestions->(
                      Js.Array.map(~f=suggestion => {
                        ...
                        <a
                          ...
                          <div
                            className={
                              "text-xs px-1 py-px ml-2 rounded font-semibold shrink-0 "
                              ++ answersClasses
                            }>
                            {answersText |> str}
                          </div>
...
let searchingIndicator = state =>
  if (state.similar.suggestions |> ArrayUtils.isEmpty && state.searching) {
              ...
              <div className="flex">
                <div className="flex-1 mr-2">
                  ...
                  <input
                    ...
                    onChange={event => {
                      let newTitle = React.Event.Form.target(event)##value;
