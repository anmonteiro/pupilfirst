...
let deleteStudentEnrollment =
    (student, coach, setDeleting, removeStudentEnrollmentCB, event) => {
  ...
  WindowUtils.confirm(
    ...
    () => {
      setDeleting(_ => true);
      let variables =
        DeleteCoachStudentEnrollmentQuery.makeVariables(
...
let make = (~student, ~coach, ~removeStudentEnrollmentCB) => {
  ...
  <div
    ...
    <div
      className="w-10 text-center flex-shrink-0 hover:text-gray-900 hover:bg-gray-50">
      <button
        title={(ts("delete") ++ " ") ++ Student.name(student)}
        onClick={deleteStudentEnrollment(
          student,
          coach,
          setDeleting,
          removeStudentEnrollmentCB,
        )}
        className="p-3">
