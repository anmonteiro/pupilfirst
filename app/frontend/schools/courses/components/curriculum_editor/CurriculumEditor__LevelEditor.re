[%%raw {js|import "./CurriculumEditor__LevelEditor.css"|js}];
...
let reducer = (state, action) =>
  ...
  | SelectCourseToCloneInto(cloneIntoCourseId) => {
      ...state,
      cloneIntoCourseId,
    }
  };
      ...
      {switch (level) {
       ...
       | None =>
         <button
           disabled={saveDisabled(state)}
           onClick={_event =>
             createLevel(course, updateLevelsCB, state, send)
           }
           className="w-full btn btn-large btn-primary">
           {t("create_level") |> str}
         </button>
...
let actionsForm = (level, levels, state, send) => {
  ...
  let otherLevels =
    Js.Array.filter(
      ~f=l => Level.id(level) != Level.id(l) && Level.number(l) != 0,
      levels,
    );
  <div className=?visibiltyClass>
    <div className="mt-5">
...
let tab = (tab, state, send) => {
  ...
  let classes =
    if (selected) {
      defaultClasses ++ " level-editor__tab--selected";
    } else {
      defaultClasses;
    };
