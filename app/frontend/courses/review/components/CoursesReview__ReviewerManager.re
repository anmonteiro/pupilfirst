...
let assignReviewer = (submissionId, setSaving, updateReviewerCB) => {
  ...
  |> Js.Promise.then_(response => {
       updateReviewerCB(
         Some(UserProxy.makeFromJs(response##assignReviewer##reviewer)),
       );
       setSaving(_ => false);
       Js.Promise.resolve();
     })
...
let reassignReviewer = (submissionId, setSaving, updateReviewerCB) => {
  ...
  |> Js.Promise.then_(response => {
       updateReviewerCB(
         Some(UserProxy.makeFromJs(response##reassignReviewer##reviewer)),
       );
       setSaving(_ => false);
       Js.Promise.resolve();
     })
  ...
  <div className="w-full p-4 md:p-6 space-y-8 mx-auto">
    <div>
      (
        ...
        | Some(reviewer) => [|
            ...
            <div
              ...
              <p className="text-sm pr-4">
                {t(
                   ~variables=[|
                     (
                       "current_coach_name",
                       UserProxy.name(Reviewer.user(reviewer)),
                     ),
                   |],
              ...
              <button
                disabled=saving
                onClick={_ =>
                  reassignReviewer(submissionId, setSaving, updateReviewerCB)
                }
        ...
        | None => [|
            <div
              className="flex items-center justify-center" key="no-reviewer">
              <div
            ...
            <div
              ...
              <button
                disabled=saving
                onClick={_ =>
                  assignReviewer(submissionId, setSaving, updateReviewerCB)
                }
