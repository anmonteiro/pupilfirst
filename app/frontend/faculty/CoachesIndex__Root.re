[%%raw {js|import "./CoachesIndex__Root.css"|js}];
...
let computeInitialState = (courses, studentInCourseIds) => {
  let filterCourseIds =
    Js.Array.filter(
      ~f=
        course =>
          studentInCourseIds |> Js.Array.includes(~value=Course.id(course)),
      courses,
    )
    |> Js.Array.map(~f=Course.id);
  {
    filterInput: "",
    filterSearch: "",
...
let reducer = (state, action) =>
  ...
  | SelectFilter(selectable) =>
    ...
    | Course(course) => {
        ...
        filterInput: "",
        filterCourseIds:
          Belt.Set.String.add(state.filterCourseIds, Course.id(course)),
      }
    }
  | DeselectFilter(selectable) =>
    ...
    | Course(course) => {
        ...state,
        filterCourseIds:
          Belt.Set.String.remove(state.filterCourseIds, Course.id(course)),
      }
    }
...
let unselected = (input, filterCourseIds, courses) => {
  let unselectedCourses =
    ...
    |> Js.Array.map(~f=Selectable.makeCourse);
  let trimmedInput = Js.String.trim(input);
  let search =
    if (Js.String.length(trimmedInput) > 0) {
      [|Selectable.makeSearch(trimmedInput)|];
    } else {
      [||];
    };
  Js.Array.concat(search, ~other=unselectedCourses);
};
...
let applyFilter = (coaches, filterSearch, filterCourseIds) => {
  let coaches =
    if (Belt.Set.String.isEmpty(filterCourseIds)) {
      coaches;
    } else {
      Js.Array.filter(
        ~f=
  ...
  if (filterSearch == "") {
    ...
    Js.Array.filter(
      ~f=coach => Coach.name(coach) |> StringUtils.includes(filterSearch),
      coaches,
    );
  };
};
let selected = (courses, filterSearch, filterCourseIds) => {
  let search =
    if (filterSearch == "") {
      ...
      [|Selectable.makeSearch(filterSearch)|];
    };
  let selectedCourses =
    Js.Array.filter(
      ~f=course => filterCourseIds->(Belt.Set.String.has(Course.id(course))),
      courses,
    )
    |> Js.Array.map(~f=Selectable.makeCourse);
  Js.Array.concat(selectedCourses, ~other=search);
};
[@react.component]
let make = (~subheading, ~coaches, ~courses, ~studentInCourseIds) => {
    ...
    | ["coaches", coachIdParam, ..._] =>
      ...
      ->(
          Belt.Option.flatMap(coachId =>
            coaches |> Js.Array.find(~f=coach => Coach.id(coach) == coachId)
          )
        )
      ->(
  ...
  let filteredCoaches =
    applyFilter(coaches, state.filterSearch, state.filterCourseIds);
  let coachesToShow =
    if (ArrayUtils.isEmpty(filteredCoaches)) {
      coaches;
    } else {
      filteredCoaches;
    };
  <div>
    ...
    <div className="max-w-5xl mx-auto px-4">
      ...
      <div
        ...
        {coachesToShow |> Js.Array.map(~f=card) |> React.array}
      </div>
    </div>
  </div>;
};
