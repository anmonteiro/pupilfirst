...
type nonrec progressionBehavior = [ | `Limited | `Unlimited | `Strict];
type nonrec state = {
  name: string,
  description: string,
  hasNameError: bool,
...
let reducer = (state, action) =>
  ...
  | UpdatePublicPreview(publicPreview) => {
      ...state,
      publicPreview,
      dirty: true,
    }
  ...
  | SetDefaultCohort(defaultCohort) => {
      ...state,
      defaultCohort: Some(defaultCohort),
      dirty: true,
    }
  };
...
let createCourse = (state, send, reloadCoursesCB) => {
  ...
  let variables =
    CreateCourseQuery.makeVariables(
      ~name=state.name,
      ~description=state.description,
      ~about=?
        if (String.trim(state.about) === "") {
          None;
        } else {
          Some(state.about);
        },
...
let updateCourse = (state, send, updateCourseCB, course) => {
  ...
  let variables =
    UpdateCourseQuery.makeVariables(
      ~id=Course.id(course),
      ~name=state.name,
      ~description=state.description,
      ~about=?
      ...
      ~processingUrl=?processingUrl(state),
      ~defaultCohortId=
        state.defaultCohort->(Belt.Option.mapWithDefault("", Cohort.id)),
      (),
    );
...
let archiveCourse = (send, reloadCoursesCB, course) => {
  ...
  |> Js.Promise.then_(result => {
       if (result##archiveCourse##success) {
         reloadCoursesCB();
       } else {
         send(FailSaving);
       };
...
let unarchiveCourse = (send, reloadCoursesCB, course) => {
  ...
  |> Js.Promise.then_(result => {
       if (result##unarchiveCourse##success) {
         reloadCoursesCB();
       } else {
         send(FailSaving);
       };
...
let booleanButtonClasses = bool => {
  ...
  classes ++ (if (bool) {" toggle-button__button--active"} else {""});
};
let publicSignupField = (publicSignup, send) =>
  <div className="flex items-center mt-5">
    ...
    <div
      id="public-signup"
      className="flex toggle-button__group shrink-0 rounded-lg">
      <button
...
let featuredButton = (featured, send) =>
  <div className="flex items-center mt-5">
    ...
    <div
      id="featured" className="flex toggle-button__group shrink-0 rounded-lg">
      <button
    ...
    {ReactUtils.nullUnless(
       <div>
         <input
           ...
           onChange={event =>
             send(
               UpdateProcessingUrl(React.Event.Form.target(event)##value),
             )
           }
         />
         <School__InputGroupError
           message={t("processing_url.error")}
           active={UrlUtils.isInvalid(true, state.processingUrl)}
         />
       </div>,
...
let courseHighlights = (highlights, send) =>
  <div className="mt-5">
    <label
      className="tracking-wide text-xs font-semibold" htmlFor="highlights">
      {t("course_highlights.label")->str}
    </label>
    <div>
      <CourseEditor__HighlightsEditor
        highlights
        updateHighlightsCB={h => send(UpdateHighlights(h))}
      />
    </div>
  </div>;
...
let computeInitialState = course =>
  ...
  | Some(course) => {
      ...
      processingUrl:
        Belt.Option.getWithDefault(Course.processingUrl(course), ""),
...
let handleSelectProgressionLimit = (send, event) => {
  ...
  | "1"
  | "2"
  | "3" => send(UpdateProgressionLimit(int_of_string(target##value)))
  | otherValue =>
...
let progressionBehaviorButtonClasses =
  ...
  let defaultClasses =
    additionalClasses
    ++ " w-1/3 relative border font-semibold focus:outline-none rounded px-5 py-4 md:px-8 md:py-5 items-center cursor-pointer text-center bg-gray-50 hover:bg-gray-300 focus:bg-gray-300 focus:ring-2 focus:ring-focusColor-500 ";
  defaultClasses
  ++ (if (selected) {" text-primary-500 border-primary-500"} else {""});
};
let detailsTab = (state, send, course, updateCourseCB, reloadCoursesCB) =>
  <div>
    <div className="mt-5">
      ...
      />
      <School__InputGroupError
        message={t("course_name.error_message")}
    ...
    <div className="mt-5">
      ...
      <input
        ...
        maxLength=150
        onChange={event =>
          updateDescription(send, React.Event.Form.target(event)##value)
        }
      />
    </div>
    <School__InputGroupError
      message={t("course_description.error_message")}
      active={state.hasDescriptionError}
    />
    <School__InputGroupError
      message={t("enter_date")}
      active={state.hasDateError}
    />
    <div className="mt-5">
      <label
        className="tracking-wide text-xs font-semibold" htmlFor="course-about">
      ...
      <div className="flex mt-2">
        <button
          ...
          <div className="font-bold text-xl">
            {t("progression_behavior.limited.title")->str}
          </div>
          <div className="text-xs mt-2">
            <div>
              {t("progression_behavior.limited.description_start")->str}
            </div>
            <select
              ...
              <option value="1">
                {t("progression_behavior.limited.once")->str}
              </option>
              <option value="2">
                {t("progression_behavior.limited.twice")->str}
              </option>
              <option value="3">
                {t("progression_behavior.limited.thrice")->str}
              </option>
            </select>
            <div>
              {t("progression_behavior.limited.description_end")->str}
            </div>
          </div>
        </button>
        <button
          ...
          <span className="text-xs">
            {t("progression_behavior.unlimited.description")->str}
          </span>
        </button>
        <button
          ...
          <div className="font-bold text-xl">
            {t("progression_behavior.strict.title")->str}
          </div>
          <span className="text-xs">
            {t("progression_behavior.strict.description")->str}
          </span>
        </button>
      </div>
    </div>
...
let cloneButtonIcons = saving =>
  if (saving) {"fas fa-spinner fa-spin"} else {"fas fa-copy"};
let submitButtonIcons = saving =>
  if (saving) {"fas fa-spinner fa-spin"} else {"fa fa-exclamation-triangle"};
    ...
    <div className="mt-2">
      ...
      <div>
        <button
          ...
          <span className="ml-2">
            {t("actions.clone_course.button_text")->str}
          </span>
        </button>
      </div>
    </div>
    {if (Belt.Option.isSome(Course.archivedAt(course))) {
       <div className="mt-2">
         <label className="tracking-wide text-xs font-semibold">
         ...
         <div>
           <button
             ...
             <span className="ml-2">
               {t("actions.unarchive_course.button_text")->str}
             </span>
           </button>
         </div>
       </div>;
       ...
       <div className="mt-2">
         ...
         <div>
           <button
             ...
             <FaIcon classes={submitButtonIcons(state.saving)} />
             <span className="ml-2">
               {t("actions.archive_course.button_text")->str}
             </span>
           </button>
         </div>
       </div>;
             ...
             <div className="w-full">
               <div
                 ...
                 <button
                   ...
                   className={
                     "-ml-px "
                     ++ selectedTabClasses(selectedTab == ActionsTab)
                   }
        ...
        {switch (course) {
         | Some(c) =>
           [|
             <div
               key="actions-tab"
               className={tabItemsClasses(selectedTab == ActionsTab)}>
               {actionsTab(state, send, reloadCoursesCB, c)}
             </div>,
             <div
               key="images-tab"
               className={tabItemsClasses(selectedTab == ImagesTab)}>
