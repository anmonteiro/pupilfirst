  ...
  if (currentUser) {
    <span
      dangerouslySetInnerHTML={DOMPurify.sanitizedHTMLOpt(
        t(
          ~variables=[|
            ("name", IssuedCertificate.profileName(issuedCertificate)),
          |],
          "heading",
        ),
...
let issuedToName = issuedCertificate => {
  ...
  if (issuedTo == profileName) {
    ...
    ++ "<i className=\"ml-1 fas fa-exclamation-circle\" /></a>";
  };
};
[@react.component]
         ...
         <div
           ...
           <div
             ...
             dangerouslySetInnerHTML={DOMPurify.sanitizedHTMLOpt(
               t(
                 ~variables=[|
                   ("name", IssuedCertificate.issuedTo(issuedCertificate)),
                 |],
  ...
  | Print =>
    <div className="flex flex-col items-center">
      <button
        ...
        <span className="ml-1"> {t("cancel")->str} </span>
      </button>
      <IssuedCertificate__Root issuedCertificate verifyImageUrl />
    </div>
  };
