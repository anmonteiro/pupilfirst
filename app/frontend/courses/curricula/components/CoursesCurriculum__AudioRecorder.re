type nonrec audioRecorderControls = {
  url: option(string),
  recording: bool,
  blobSize: option(int),
...
[@mel.module "./CoursesCurriculum__AudioNavigator"]
external audioRecorder: (string, bool => unit) => audioRecorderControls =
  "audioRecorder";
let t = I18n.t(~scope="components.CoursesCurriculum__AudioRecorder");
[@react.component]
let make = (~attachingCB, ~attachFileCB, ~preview) => {
  let audioRecorder =
    audioRecorder(AuthenticityToken.fromHead(), attachingCB);
  React.useEffect1(
    () => {
    ...
    {if (audioRecorder.recording) {
       ...
       <div
         ...
         {switch (audioRecorder.url) {
          | None => React.null
          | Some(url) =>
            <audio src=url controls=true className="pt-3 md:pt-0 md:pl-4" />
          }}
         {switch (audioRecorder.url) {
          | None => React.null
          | Some(_) =>
            <div
              ...
              onClick={_e => audioRecorder.downloadBlob()}>
