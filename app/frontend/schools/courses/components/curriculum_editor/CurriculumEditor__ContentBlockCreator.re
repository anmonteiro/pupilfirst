...
[%%raw {js|import "./CurriculumEditor__ContentBlockCreator.css"|js}];
...
let computeInitialState = isAboveTarget => {
  ui: if (isAboveTarget) {Hidden} else {BlockSelector},
  saving: false,
  error: None,
  uploadProgress: None,
  videoTitle: "",
...
let createMarkdownContentBlock =
  ...
  let variables =
    CreateMarkdownContentBlock.makeVariables(
      ~targetId,
      ~aboveContentBlockId?,
      (),
    );
...
let videoInputId = aboveContentBlock =>
  elementId("markdown-block-vimeo-input-", aboveContentBlock);
let videoFormId = aboveContentBlock =>
  elementId("markdown-block-vimeo-form-", aboveContentBlock);
...
let audioFormId = aboveContentBlock =>
  elementId("markdown-block-audio-form-", aboveContentBlock);
let audioInputId = aboveContentBlock =>
  elementId("markdown-block-audio-input-", aboveContentBlock);
...
let validEmbedUrl = url =>
  Belt.Array.some(embedUrlRegexes, regex => regex->(Js.Re.test(~str=url)));
let handleCreateEmbedContentBlock =
  ...
  if (url |> validEmbedUrl) {
    ...
    let targetId = target |> Target.id;
    let variables =
      CreateEmbedContentBlock.makeVariables(
        ~targetId,
        ~aboveContentBlockId?,
        ~url,
        ~requestSource,
...
let uploadFile =
  ...
  | `VideoEmbed =>
    let size = file##size;
    let title =
      if (String.trim(state.videoTitle) == "") {
        None;
      } else {
        Some(state.videoTitle);
      };
    let description =
      if (String.trim(state.videoDescription) == "") {
        None;
      } else {
        Some(state.videoDescription);
      };
    let variables =
      CreateVimeoVideo.makeVariables(
...
let maxVideoSize = vimeoPlan =>
  ...
  | Some(plan) =>
    ...
    | Pro
    | Business
    | Premium => 5.0 *. 1024.0 *. 1024.0 *. 1024.0
    }
...
let handleFileInputChange =
  ...
  | files =>
    ...
    let error =
      switch (blockType) {
      | `File =>
        if (FileUtils.isInvalid(file)) {
          Some(t("file.upload_size_warning"));
        } else {
          None;
        }
      | `Image =>
        if (FileUtils.isInvalid(~image=true, file)) {
          Some(t("image.invalid_image_warning"));
        } else {
          None;
        }
      | `VideoEmbed =>
        ...
        switch (
          ...
          FileUtils.hasValidFloatSize(~maxSize=maxVideoSize, file),
        ) {
        ...
        | (true, false) =>
          Some(
            t(
              ~variables=[|
                ("maximumVideoSize", maxVideoSizeString(vimeoPlan)),
              |],
      ...
      | `Audio =>
        switch (
          ...
          FileUtils.hasValidSize(~maxSize=10 * 1024 * 1024, file),
        ) {
...
let closeUploadFormButton = (send, aboveContentBlock) => {
  let id =
    aboveContentBlock->(
                         Belt.Option.mapWithDefault("button", ContentBlock.id)
                       );
  topButton(
...
let buttonAboveContentBlock = (state, send, aboveContentBlock) =>
  ...
  | (Hidden, None)
  | (BlockSelector, None) => <div className="h-10" />
        ...
        {switch (state.ui) {
         ...
         | EmbedForm(url) =>
           <div
             ...
             <HelpIcon
               className="ml-2 text-xs" link={t("embed_url.help_url")}>
