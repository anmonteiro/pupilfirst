...
let reducer = (state, action) =>
  ...
  | DeSelectStudent(student) => {
      ...
      selectedStudent:
        ...
        ->(
            Js.Array.filter(~f=s => UserProxy.id(s) != UserProxy.id(student))
          ),
      dirty: true,
    }
  | SelectedStudent(student) => {
      ...state,
      selectedStudent:
        state.selectedStudent->(Js.Array.concat(~other=[|student|])),
      dirty: true,
    }
  };
...
let createTeam = (state, send, cohortId, courseId) => {
  send(SetSaving);
  let variables =
    CreateTeamQuery.makeVariables(
      ~name=state.name,
      ~studentIds=state.selectedStudent->(Js.Array.map(~f=UserProxy.id)),
      ~cohortId,
      (),
    );
...
let disabled = state =>
  ...
  || Js.Array.length(state.selectedStudent) < 2;
module Selectable = {
...
let loadCohortsData = (courseId, send) => {
  send(SetLoading);
  TeamsEditorBaseDataQuery.fetch(
    TeamsEditorBaseDataQuery.makeVariables(~courseId, ()),
  )
...
let selectedStudent = (send, student) => send(SelectedStudent(student));
let deSelectStudent = (send, student) => send(DeSelectStudent(student));
[@react.component]
    ...
    <div className="max-w-4xl 2xl:max-w-5xl mx-auto px-4">
      ...
      <div className="pt-5 flex flex-col">
        ...
        <Dropdown
          ...
          disabled={
            state.selectedStudent->ArrayUtils.isNotEmpty
            || Belt.Option.isSome(team)
          }
            ...
            | None =>
              <div className="py-6">
                <button
                  ...
                  disabled={disabled(state)}
                  onClick={_e =>
                    createTeam(state, send, Cohort.id(cohort), courseId)
                  }>
                  {t("add_new_team")->str}
                </button>
              </div>
