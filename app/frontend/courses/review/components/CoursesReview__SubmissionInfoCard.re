[%%raw {js|import "./CoursesReview__SubmissionInfoCard.css"|js}];
...
let cardClasses = (submission, selected) =>
  (
    ...
    ++ (
      if (selected) {"border border-primary-400 "} else {
  ...
  ++ (
    ...
    | (None, None, None) => "submission-info__tab-pending focus:ring focus:ring-focusColor-400 shadow hover:shadow-lg"
    | (None, None, Some(_)) => "submission-info__tab-rejected focus:ring focus:ring-focusColor-400 shadow hover:shadow-lg"
    | (None, Some(_), None)
    | (None, Some(_), Some(_)) => "submission-info__tab-completed focus:ring focus:ring-focusColor-400 shadow hover:shadow-lg"
    }
  );
    ...
    <div className="flex flex-col md:pr-6">
      <h2 className="font-semibold text-sm leading-tight">
        <p className="hidden md:block">
          {(
             (t("submission_hash") ++ " #") ++ string_of_int(submissionNumber)
           )
           ->str}
        </p>
