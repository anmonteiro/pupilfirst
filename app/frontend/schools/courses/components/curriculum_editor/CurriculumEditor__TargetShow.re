[%%raw {js|import "./CurriculumEditor__TargetShow.css"|js}];
...
let updateSortIndex = (targets, target, up, updateTargetSortIndexCB) => {
  let index = Js.Array.indexOf(~value=target, targets);
  let newTargets =
    if (up) {
      ArrayUtils.swapUp(index, targets);
    } else {
      ArrayUtils.swapDown(index, targets);
    };
  let targetIds = newTargets |> Js.Array.map(~f=Target.id);
    ...
    {if (Js.Array.length(targets) > 1) {
       <div
         ...
         <button
           ...
           className={
             "target-group__target-reorder-up flex items-center justify-center cursor-pointer w-9 h-9 p-1 text-gray-400 hover:bg-gray-50 focus:outline-none focus:text-primary-500"
         ...
         <button
           ...
           className={
             "target-group__target-reorder-down flex items-center justify-center cursor-pointer w-9 h-9 p-1 border-t border-transparent text-gray-400 hover:bg-gray-50 focus:outline-none focus:text-primary-500"
             ++ sortIndexHiddenClass(index + 1 == Js.Array.length(targets))
           }
           onClick={_ =>
             updateSortIndex(targets, target, false, updateTargetSortIndexCB)
           }>
