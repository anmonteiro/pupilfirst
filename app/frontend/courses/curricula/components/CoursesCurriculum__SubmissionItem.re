...
let showLink = (value, id, updateResultCB) =>
  <div>
    <input
      ...
      onChange={e =>
        updateResultCB(
          ChecklistItem.Link(React.Event.Form.target(e)##value),
        )
      }
...
let showShortText = (value, id, updateResultCB) =>
  <div>
    <input
      ...
      onChange={e =>
        updateResultCB(
          ChecklistItem.ShortText(React.Event.Form.target(e)##value),
        )
      }
...
let longTextWarning = value => {
  ...
  if (showWarning) {
    <div className="flex justify-between items-center mt-1">
      ...
      <div
        className={
          "shrink-1 text-tiny font-semibold px-1 py-px border border-transparent rounded "
          ++ colors
        }>
...
let attachRecordingFile = (updateResultCB, attachingCB, id, filename) => {
  attachingCB(false);
  updateResultCB(ChecklistItem.AudioRecord({id, name: filename}));
};
let removeFile = (updateResultCB, files, id) =>
  updateResultCB(
    ChecklistItem.Files(
      files |> Js.Array.filter(~f=a => a |> ChecklistItem.fileId != id),
    ),
  );
...
let showAudioRecorder = (attachingCB, updateResultCB, preview) =>
  <CoursesCurriculum__AudioRecorder
    attachingCB
    attachFileCB={attachRecordingFile(updateResultCB, attachingCB)}
    preview
  />;
[@react.component]
  ...
  <div className="mt-4" ariaLabel=id>
    ...
    <div className="md:pl-7 pt-2 pr-0 pb-4">
      {switch (checklistItem |> ChecklistItem.result) {
       | Files(files) =>
         showFiles(files, preview, id, attachingCB, updateResultCB, index)
       ...
       | AudioRecord(_) =>
         showAudioRecorder(attachingCB, updateResultCB, preview)
       }}
    </div>
  </div>;
};
