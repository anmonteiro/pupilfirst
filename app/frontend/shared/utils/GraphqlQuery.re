...
module Extender = (M: X) => {
  ...
  let decodeErrors = json =>
    Json.Decode.(json |> array(field("message", string)));
      ...
      |> Js.Promise.then_(json =>
           ...
           | Some(obj) =>
             ...
             | Some(errors) =>
               ...
               errors
               |> decodeErrors
               |> Js.Array.forEach(~f=e => Notification.error("Error", e));
  ...
  let make = (~notify=true, ~notifyOnNotFound=true, variables) =>
    query(notify, notifyOnNotFound, variables)
    |> Js.Promise.then_(data =>
         tToJsObject(M.unsafe_fromJson(data) |> M.parse)
         |> Js.Promise.resolve
       );
};
