  ...
  | _ =>
    <button
      ...
      {switch (saving, linkToComplete) {
       | (true, _) =>
         completeButtonText(tr("saving"), "fas fa-spinner fa-spin")
       | (false, Some(_)) =>
         completeButtonText(tr("visit_complete"), "fas fa-external-link-alt")
       | (false, None) =>
         completeButtonText(tr("mark_complete"), "fas fa-check-square")
       }}
    </button>
  };
