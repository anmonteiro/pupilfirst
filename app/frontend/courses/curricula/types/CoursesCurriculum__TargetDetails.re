...
let decode = json =>
  Json.Decode.{
    ...
    submissions:
      json
      |> field("submissions", array(CoursesCurriculum__Submission.decode)),
    feedback:
      json |> field("feedback", array(CoursesCurriculum__Feedback.decode)),
    quizQuestions:
      json
      |> field(
           "quizQuestions",
           array(CoursesCurriculum__QuizQuestion.decode),
         ),
    contentBlocks:
      json |> field("contentBlocks", array(ContentBlock.decode)),
    communities:
      json
      |> field("communities", array(CoursesCurriculum__Community.decode)),
    linkToComplete:
      json |> field("linkToComplete", nullable(string)) |> Js.Null.toOption,
    evaluated: json |> field("evaluated", bool),
    grading:
      json |> field("grading", array(CoursesCurriculum__Grade.decode)),
