...
let evaluationCriterionForGrade = (grade, evaluationCriteria) =>
  ...
  |> ArrayUtils.unsafeFind(
       ec =>
         CoursesReport__EvaluationCriterion.id(ec)
...
let computeAverageQuizScore = quizScores => {
  let sumOfPercentageScores =
    ...
    |> Js.Array.reduce(~f=(a, b) => a +. b, ~init=0.0);
  sumOfPercentageScores /. (quizScores |> Array.length |> float_of_int);
};
let averageQuizScore = t =>
  if (t.quizScores |> ArrayUtils.isEmpty) {
    None;
  } else {
    Some(computeAverageQuizScore(t.quizScores));
  };
let makeFromJs = (id, studentData) => {
  id,
  evaluationCriteria:
    studentData##evaluationCriteria
    |> CoursesReport__EvaluationCriterion.makeFromJs,
