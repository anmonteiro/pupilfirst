[@mel.module "./images/set-new-password-icon.svg"]
...
let updatePassword =
    (authenticityToken, token, newPassword, confirmPassword, setSaving) => {
  ...
  <div className="pt-4 pb-5 md:px-9 items-center max-w-sm mx-auto">
    <div>
      ...
      <input
        ...
        onChange={event =>
          setNewPassword(React.Event.Form.target(event)##value)
        }
      />
    </div>
    <div className="mt-4">
      ...
      <input
        ...
        onChange={event =>
          setConfirmPassword(React.Event.Form.target(event)##value)
        }
      />
    </div>
    <button
      ...
      <FaIcon
        classes={if (saving) {"fas fa-spinner fa-spin"} else {"fas fa-lock"}}
      />
      <span className="ml-2">
