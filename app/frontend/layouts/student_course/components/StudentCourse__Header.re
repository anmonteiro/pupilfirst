...
[%%raw {js|import "./StudentCourse__Header.css"|js}];
[%%raw {js|import "~/shared/styles/background_patterns.css"|js}];
...
let courseNameContainerClasses = additionalLinks =>
  ...
  ++ (
    if (additionalLinks->ArrayUtils.isEmpty) {
...
let renderCourseSelector =
  ...
  let otherCourses =
    Js.Array.filter(~f=c => CourseInfo.id(c) != currentCourseId, courses);
