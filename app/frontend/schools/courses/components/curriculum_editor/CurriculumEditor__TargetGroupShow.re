...
let archivedClasses = archived =>
  ...
  ++ (if (archived) {"target-group__header--archived"} else {""});
let updateSortIndex =
    (targetGroups, targetGroup, up, updateTargetGroupSortIndexCB) => {
  let index = Js.Array.indexOf(~value=targetGroup, targetGroups);
  let newTargetGroups =
    if (up) {
  ...
  let targetGroupIds = Js.Array.map(~f=TargetGroup.id, newTargetGroups);
...
let sortIndexHiddenClass = bool => if (bool) {" invisible"} else {""};
[@react.component]
let make =
    (
      ~targetGroup,
  ...
  let targetsInGroup =
    ...
    |> Js.Array.filter(~f=target =>
         Target.targetGroupId(target) == TargetGroup.id(targetGroup)
       )
    |> Target.sort;
  let targetsToDisplay =
    if (showArchived) {
      ...
      targetsInGroup
      |> Js.Array.filter(~f=target =>
           !(Target.visibility(target) == Archived)
         );
    };
  ...
  <div
    ...
    <div
      ...
      {if (targetGroups |> Js.Array.length > 1) {
         <div
           ...
           <button
             ...
             className={
               "target-group__group-reorder-up flex items-center justify-center cursor-pointer w-9 h-9 p-1 text-gray-400 hover:bg-gray-50 focus:outline-none focus:bg-gray-50 focus:text-primary-500"
           ...
           <button
             ...
             className={
               "target-group__group-reorder-down flex items-center justify-center cursor-pointer w-9 h-9 p-1 text-gray-400 hover:bg-gray-50 focus:outline-none focus:bg-gray-50 focus:text-primary-500"
               ++ sortIndexHiddenClass(
                    index + 1 == Js.Array.length(targetGroups),
                  )
             }
             onClick={_ =>
               updateSortIndex(
    ...
    {if (targetGroupArchived) {
       ...
       <div
         ...
         <input
           ...
           onChange={event =>
             send(UpdateTargetTitle(React.Event.Form.target(event)##value))
           }
