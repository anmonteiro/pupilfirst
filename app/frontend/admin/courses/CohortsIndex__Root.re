...
let reducer = (state, action) =>
  ...
  | 
    ...
    let updatedStudent =
      ...
      | LoadingMore =>
        Js.Array.concat(PagedCohorts.toArray(state.cohorts), ~other=students)
  ...
  |> Js.Promise.then_((response: CourseCohortsQuery.t) => {
       send(
         ...
         LoadCohorts(
           ...
           response.cohorts.nodes
           ->(
               Js.Array.map(~f=(c: CourseCohortsQuery.t_cohorts_nodes) =>
                 {
                   id: c.id,
                   name: c.name,
...
let cohortsList = cohorts =>
  <div className="space-y-4">
    {cohorts
     ->(
         Js.Array.map(~f=cohort =>
           <Spread props={"data-cohort-name": cohort.name}>
             <div
               key={cohort.id}
               className="cohorts-container p-6 bg-white rounded-lg shadow">
               <div className="flex items-center justify-between">
                 ...
                 <div>
                   <Link
                     href={
                       ({js|/school/cohorts/|js} ++ cohort.id)
                       ++ {js|/details|js}
                     }
               ...
               <div className="flex gap-6 flex-wrap mt-6">
                 <div>
                   ...
                   <p
                     className="pr-3 mt-2 border-r-2 border-gray-200 font-semibold">
                 ...
                 cohort.endsAt
                 ->(
                     Belt.Option.mapWithDefault(React.null, endsAt =>
                       <div>
                         <p className="pr-6 text-sm text-gray-500 font-medium">
      ...
      <div className="max-w-4xl 2xl:max-w-5xl mx-auto px-4">
        <div className="flex gap-2 items-end justify-between">
          ...
          <Link
            ...
            href={
              ({js|/school/courses/|js} ++ courseId) ++ {js|/cohorts/new|js}
            }>
            <PfIcon className="if i-plus-regular" />
            <span className="inline-block pl-2">
              {str(t("add_new_cohort"))}
            </span>
          </Link>
        </div>
