[%%raw {js|import "./AppRouter__Header.css"|js}];
  ...
  <div
    ...
    <div className="flex items-center justify-center">
      <a
        ...
        <span className="ml-2"> {t("sign_out")->str} </span>
      </a>
    </div>
  </div>;
let signInLink = () =>
  <div
    ...
    <div className="flex items-center justify-center">
      <a
        ...
        href="/users/sign_in">
        <FaIcon classes="fas fa-power-off" />
        <span className="ml-2"> {t("sign_in")->str} </span>
      </a>
    </div>
  </div>;
...
let headerLinks = (links, isLoggedIn, user, hasNotifications) => {
  let (visibleLinks, dropdownLinks) =
    switch (Array.to_list(links), isMobile()) {
    | (links, true) => (Array.of_list(links), [||])
    | ([l1, l2, l3, l4, l5, ...rest], false) => (
        [|l1, l2, l3|],
        Js.Array.concat(Array.of_list(rest), ~other=[|l4, l5|]),
      )
    | (fourOrLessLinks, false) => (Array.of_list(fourOrLessLinks), [||])
    };
  visibleLinks
  ->(Js.Array.mapi(~f=(l, index) => headerLink(string_of_int(index), l)))
  ->(
      Js.Array.concat(
        ~other=[|
          <AppRouter__Dropdown links=dropdownLinks key="more-links" />,
        |],
      )
    )
  ->(
...
let make = (~school, ~currentUser) => {
  ...
  let hasNotifications =
    Belt.Option.mapWithDefault(currentUser, false, User.hasNotifications);
  <div>
      ...
      <nav className="flex justify-between items-center">
        <div className="flex w-full items-center justify-between">
          <a
            ...
            href={
                   if (Belt.Option.isSome(currentUser)) {
                     "/dashboard";
                   } else {
                     "/";
                   }
          ...
          {ReactUtils.nullUnless(
             <div className="flex items-center space-x-2">
               ...
               <div onClick={_ => toggleMenuHidden(menuHidden => !menuHidden)}>
                 <div
                   className={
                     "app-router-header__menu-btn cursor-pointer hover:bg-gray-50 w-8 h-8 text-center relative focus:outline-none rounded-lg "
                     ++ (if (menuHidden) {""} else {"open"})
                   }>
