...
let search =
  ...
  | searchString =>
    let allTags =
      Js.Array.map(
        ~f=String.lowercase_ascii,
        Js.Array.concat(selectedTags, ~other=unselectedTags),
      );
    let initial =
      if (allowNewTags
          && !Array.mem(searchString->String.lowercase_ascii, allTags)) {
        [|
          <button
...
let make =
  ...
  <div className="mt-1">
    ...
    <input
      value=state
      onChange={event => send(React.Event.Form.target(event)##value)}
