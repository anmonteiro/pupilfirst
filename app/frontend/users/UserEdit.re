...
let reducer = (state, action) =>
  ...
  | SetDisableUpdateEmail(disableEmailInput) => {...state, disableEmailInput}
    ...
    Api.sendFormData(
      ...
      json => {
        Notification.success(
          ...
          t("avatar_uploaded_notification"),
        );
...
let initiateAccountDeletion = (state, send) => {
  ...
  |> Js.Promise.then_((result: InitiateAccountDeletionQuery.t) => {
       if (result.initiateAccountDeletion.success) {
...
let hasInvalidPassword = state =>
  if (state.newPassword == ""
      ...
      && state.newPassword->String.length >= 8) {
    false;
  } else {
    true;
  };
let saveDisabled = state =>
  ...
  || state.name->String.trim->String.length == 0
  || !state.dirty;
      ...
      <div className="px-4 py-5 sm:p-6">
            ...
            <div className="mt-6">
              ...
              <div>
                <textarea
                  ...
                  rows=3
                  onChange={event =>
                    send(UpdateAbout(React.Event.Form.target(event)##value))
                  }
                  className="appearance-none block text-sm w-full shadow-sm border border-gray-300 rounded px-4 py-2 my-2 leading-relaxed focus:outline-none focus:border-transparent focus:ring-2 focus:ring-focusColor-500"
            ...
            <div className="mt-6">
              ...
              <div className="mt-2 flex items-stretch gap-2">
                <input
                  ...
                  onChange={event =>
                    send(UpdateEmail(React.Event.Form.target(event)##value))
                  }
                  ...
                  required=true
                />
                {if (state.disableEmailInput) {
                   <button
                     className="btn btn-primary"
                     onClick={_evt => send(SetDisableUpdateEmail(false))}>
                     {ts("edit")->str}
                   </button>;
                 } else {
                   <div className="flex gap-2">
                     ...
                     <button
                       ...
                       disabled={
                         EmailUtils.isInvalid(false, state.email)
                         || state.email == email
                       }>
                       {ts("update")->str}
                     </button>
          ...
          <div className="mt-5 md:mt-0 w-full md:w-2/3">
            ...
            <div className="mt-6">
              ...
              <input
                ...
                onChange={event =>
                  send(
                    UpdateNewPassword(React.Event.Form.target(event)##value),
                  )
                }
                className="appearance-none block text-sm w-full shadow-sm border border-gray-300 rounded px-4 py-2 my-2 leading-relaxed focus:outline-none focus:border-transparent focus:ring-2 focus:ring-focusColor-500"
        ...
        <div className="flex flex-col md:flex-row mt-10 md:mt-12">
          <div className="w-full md:w-1/3 pr-4">
            ...
            <p className="mt-1 text-sm text-gray-600">
              {t("update_email_notifications")->str}
            </p>
          </div>
          <div className="mt-5 md:mt-0 w-full md:w-2/3">
            <p className="font-semibold"> "Community Digest"->str </p>
            <p className="text-sm text-gray-600">
              {t("community_digest_emails")->str}
            </p>
            <div className="mt-6">
          ...
          <div className="mt-5 md:mt-0 w-full md:w-2/3">
            ...
            <div className="mt-6">
              <select
                ...
                value={state.locale}
                onChange={event =>
                  send(UpdateLocale(React.Event.Form.target(event)##value))
                }
                ...
                {availableLocales
                 ->(
                     Js.Array.map(~f=availableLocale =>
                       <option
                         key=availableLocale
      ...
      <div className="px-4 py-5 sm:p-6">
        <div className="flex flex-col md:flex-row">
          ...
          <div className="mt-5 md:mt-0 w-full md:w-2/3">
            ...
            <div className="mt-4">
              {if (isSchoolAdmin || hasValidDeleteAccountToken) {
                 <div
