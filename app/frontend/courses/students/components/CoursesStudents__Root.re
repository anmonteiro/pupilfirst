[%%raw {js|import "./CoursesStudents__Root.css"|js}];
open CoursesStudents__Types;
let str = React.string;
let tr = I18n.t(~scope="components.CoursesStudents__Root");
let ts = I18n.t(~scope="shared");
type nonrec coachNoteFilter = [
  | `WithCoachNotes
  | `WithoutCoachNotes
  | `IgnoreCoachNotes
];
...
let getStudents = (send, courseId, cursor, ~loadingMore=false, params) => {
  ...
  Webapi.Url.URLSearchParams.append(
    "request_origin",
    "review_interface",
    params,
  );
  ...
  |> Js.Promise.then_((response: StudentsQuery.t) => {
       ...
       let students =
         nodes->(
                  Js.Array.map(~f=(s: StudentsQuery.t_courseStudents_nodes) =>
                    StudentInfo.make(
                      ~id=s.id,
                      ~taggings=s.taggings,
       ...
       let studentDistribution =
         response.studentDistribution
         ->(
             Belt.Option.map(p =>
               p->(
                    Js.Array.map(~f=(d: StudentsQuery.t_studentDistribution) =>
