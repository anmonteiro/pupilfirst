...
let unselectedlevels = (levels, levelId) =>
  levelId->(
             Belt.Option.mapWithDefault(levels, levelId =>
               levels |> Js.Array.filter(~f=l => l |> Level.id != levelId)
             )
           );
let selectedLevel = (levels, levelId) =>
  levelId->(
             Belt.Option.mapWithDefault([||], levelId =>
               [|
                 levelId
                 |> Level.unsafeFind(
                      levels,
                      "TargetGroupEditor.selectedTargetGroup",
                    ),
               |]
             )
           );
...
let handleResponseCB =
  ...
  | Some(_) =>
    Notification.success(
      ...
      tr("target_group_updated_notificaion"),
    )
  | None =>
    Notification.success(
      ts("notifications.success"),
      tr("target_group_created_notification"),
    )
  };
            ...
            <div className="max-w-2xl pt-6 px-6 mx-auto">
              ...
              <div className="mt-5">
                <div className="flex items-center shrink-0">
                  ...
                  <div className="milestone shrink-0 overflow-hidden">
                    <button
                      onClick={_event => {
