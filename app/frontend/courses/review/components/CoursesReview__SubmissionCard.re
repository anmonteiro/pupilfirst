[@mel.module "../../shared/images/reviewed-empty.svg"]
external reviewedEmptyImage: string = "default";
[@mel.module "../images/pending-empty.svg"]
...
let submissionStatus = submission => {
  ...
  let (className, text) =
    if (IndexSubmission.pendingReview(submission)) {
      (
        classes ++ "bg-orange-100 text-orange-800",
        IndexSubmission.timeDistance(submission),
      );
...
let make = (~submissions, ~selectedTab, ~filterString) => {
  let imageSrc =
    Belt.Option.mapWithDefault(selectedTab, pendingEmptyImage, t =>
      switch (t) {
