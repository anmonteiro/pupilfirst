[%%mel.raw {js|import "./CoursesReview__ChecklistEditor.css"|js}];
...
let updateReviewChecklist =
    (targetId, reviewChecklist, send, updateReviewChecklistCB) => {
  send(UpdateSaving);

  let trimmedChecklist =
    reviewChecklist->(Js.Array.map(~f=ReviewChecklistItem.trim));
...
let updateChecklistResultTitle =
    (itemIndex, resultIndex, title, reviewChecklistItem, resultItem, send) => {
...
let updateChecklistResultFeedback =
    (itemIndex, resultIndex, feedback, reviewChecklistItem, resultItem, send) => {
  let newReviewChecklistItem =
    ReviewChecklistItem.updateChecklist(
...
let removeChecklistResult =
    (itemIndex, resultIndex, reviewChecklistItem, send) =>
  send(
    UpdateChecklistItem(
...
let invalidTitle = title => title->Js.String.trim == "";

let invalidChecklist = reviewChecklist =>
  reviewChecklist
  ->(
      Js.Array.map(~f=reviewChecklistItem =>
        invalidTitle(ReviewChecklistItem.title(reviewChecklistItem))
        || ReviewChecklistItem.result(reviewChecklistItem)
           ->(
               Js.Array.filter(~f=resultItem =>
                 invalidTitle(ReviewChecklistResult.title(resultItem))
               )
             )
           ->ArrayUtils.isNotEmpty
      )
    )
  ->(Js.Array.filter(~f=valid => valid))
  ->ArrayUtils.isNotEmpty;
...
let reducer = (state, action) =>
  ...
  | AddEmptyChecklistItem => {
      ...
      reviewChecklist:
        Js.Array.concat(
          state.reviewChecklist,
          ~other=ReviewChecklistItem.empty(),
        ),
    }
  | RemoveChecklistItem(itemIndex) => {
      ...
      reviewChecklist:
        state.reviewChecklist
        ->(Js.Array.filteri(~f=(_el, i) => i != itemIndex)),
    }
    ...
    <DisablingCover disabled={state.saving}>
      <div className="p-4 md:p-6 relative">
        <div>
          {state.reviewChecklist
           ->(
               Js.Array.mapi(~f=(reviewChecklistItem, itemIndex) =>
                 <Spread
                   ...
                   <div className="flex items-start gap-1">
                     <div
                       ...
                       ariaLabel={
                         "checklist-item-" ++ itemIndex->string_of_int
                       }>
                       <div className="flex">
                         <div className="w-full">
                           <input
                             ...
                             id={
                               ("checklist_" ++ string_of_int(itemIndex))
                               ++ "_title"
                             }
                             type_="text"
                             placeholder={t("checklist_title.placeholder")}
                             value={ReviewChecklistItem.title(
                               reviewChecklistItem,
                             )}
                             onChange={event =>
                           ...
                           <School__InputGroupError
                             ...
                             active={invalidTitle(
                               ReviewChecklistItem.title(reviewChecklistItem),
                             )}
                           />
                         </div>
                       </div>
                       <div>
                         {ReviewChecklistItem.result(reviewChecklistItem)
                          ->(
                              Js.Array.mapi(~f=(resultItem, resultIndex) => {
                                let feedback =
                                      ...
                                      <div
                                        ...
                                        <div
                                          ...
                                          <input
                                            ...
                                            type_="text"
                                            placeholder={t(
                                              "checklist_item_title.placeholder",
                                            )}
                                            value={ReviewChecklistResult.title(
                                              resultItem,
                                            )}
                                            onChange={event =>
                                          ...
                                          <div
                                            ...
                                            {controlIcon(
                                               ...
                                               ~title=
                                                 t(
                                                   "checklist_item_title.move_up_button_title",
                                                 ),
                                            ...
                                            {controlIcon(
                                               ...
                                               ~title=
                                                 t(
                                                   "checklist_item_title.move_down_button_title",
                                                 ),
                                               ~hidden=
                                                 resultIndex
                                                 == Js.Array.length(
                                            ...
                                            {controlIcon(
                                               ...
                                               ~title=
                                                 t(
                                                   "checklist_item_title.remove_button_title",
                                                 ),
                                               _ =>
                                        ...
                                        <textarea
                                          ...
                                          type_="text"
                                          placeholder={t(
                                            "checklist_item_description.placeholder",
                                          )}
                                          value=feedback
                                          onChange={event =>
                                        ...
                                        <School__InputGroupError
                                          message={t(
                                            "checklist_item_description.error_message",
                                          )}
                                          active={invalidTitle(
                     ...
                     <div
                       ...
                       {controlIcon(
                          ...
                          ~hidden=itemIndex <= 0,
                          _ =>
                          send(SwapUpReviewChecklistItem(itemIndex))
                        )}
                       {controlIcon(
                          ~icon="fa-arrow-down",
                          ...
                          ~hidden=
                            itemIndex
                            == Js.Array.length(state.reviewChecklist)
                            - 1,
                          _ =>
                          send(SwapDownReviewChecklistItem(itemIndex))
                        )}
                       {controlIcon(
                          ~icon="fa-trash-alt",
                          ~title=t("checklist_title.remove_button_title"),
                          _ =>
                          send(RemoveChecklistItem(itemIndex))
                        )}
                     </div>
                   </div>
                 </Spread>
          ...
          <div>
            <button
              ...
              <span className="bg-gray-100 py-2 w-10">
                <i className="fas fa-plus text-sm" />
              </span>
              <span className="px-3 py-2">
                {t("add_checklist_item")->str}
              </span>
            </button>
          </div>
        </div>
