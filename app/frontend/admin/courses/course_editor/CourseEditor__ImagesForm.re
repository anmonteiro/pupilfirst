...
let updateButtonText = updating =>
  if (updating) {
    t("button_text.updating");
  } else {
    t("button_text.update_images");
  };
let formId = "course-editor-form-image-form";
let filename = optionalFilename =>
  optionalFilename->(Belt.Option.getWithDefault("unknown"));
    ...
    Api.sendFormData(
      ...
      json => {
        Notification.success(
          t("notification_success.title"),
          t("notification_success.description"),
        );
    ...
    <DisablingCover disabled={state.updating}>
      <div key="course-images-editor__thumbnail" className="mt-4">
        ...
        <School__InputGroupError
          message={t("thumbnail.error_message")}
      ...
      <div key="course-images-editor__cover" className="mt-4">
        ...
        <School__InputGroupError
          message={t("cover_image.error_message")}
