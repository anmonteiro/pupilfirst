...
let getSubmissionDetails = (submissionId, setState, ()) => {
  ...
  |> Js.Promise.then_(response => {
       setState(_ =>
         Loaded(SubmissionDetails.decodeJs(response##submissionDetails))
       );
       Js.Promise.resolve();
     })
  |> ignore;
...
let updateSubmission = (setState, submissionDetails, overlaySubmission) => {
  let newSubmissionDetails =
    SubmissionDetails.updateOverlaySubmission(
      overlaySubmission,
      submissionDetails,
    );
...
let currentSubmissionIndex = (submissionId, allSubmissions) =>
  Js.Array.length(allSubmissions)
  - Js.Array.findIndex(
      ~f=s => SubmissionMeta.id(s) == submissionId,
      allSubmissions,
    );
let updateReviewChecklist = (submissionDetails, setState, reviewChecklist) =>
  setState(_ =>
    Loaded(
      SubmissionDetails.updateReviewChecklist(
        reviewChecklist,
        submissionDetails,
      ),
    )
...
let updateReviewer = (submissionDetails, setState, reviewer) =>
  setState(_ =>
    Loaded(SubmissionDetails.updateReviewer(reviewer, submissionDetails))
  );
let updateSubmissionReport = (submissionDetails, setState, submissionReport) =>
  setState(_ =>
    Loaded(
      SubmissionDetails.updateSubmissionReport(
        submissionReport,
        submissionDetails,
      ),
    )
    ...
    {switch (state) {
     | Loaded(submissionDetails) =>
       <CoursesReview__Editor
         ...
         teamSubmission={
           SubmissionDetails.students(submissionDetails)->Js.Array.length > 1
         }
         evaluationCriteria={SubmissionDetails.evaluationCriteria(
           submissionDetails,
         )}
         targetEvaluationCriteriaIds={SubmissionDetails.targetEvaluationCriteriaIds(
           submissionDetails,
         )}
         reviewChecklist={SubmissionDetails.reviewChecklist(
           submissionDetails,
         )}
         ...
         submissionReport={SubmissionDetails.submissionReport(
           submissionDetails,
         )}
         ...
         updateSubmissionReportCB={updateSubmissionReport(
           submissionDetails,
           setState,
         )}
         submissionReportPollTime={SubmissionDetails.submissionReportPollTime(
           submissionDetails,
         )}
       />
     | Loading =>
