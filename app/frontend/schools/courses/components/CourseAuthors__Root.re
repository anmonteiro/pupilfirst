...
let reducer = (state, action) =>
  ...
  | FinishDeleting(author) => {
      ...
      authors:
        state.authors
        |> Js.Array.filter(~f=a => a |> Author.id != (author |> Author.id)),
    }
  | AddAuthor(author) => {
      ...state,
      authors: state.authors |> Js.Array.concat(~other=[|author|]),
    }
  | UpdateAuthor(author) => {
      ...state,
      authors:
        state.authors
        |> Array.map(a =>
             if (a |> Author.id == (author |> Author.id)) {
               author;
             } else {
               a;
             }
...
let removeCourseAuthor = (send, author, event) => {
  ...
  WindowUtils.confirm(
    (t("window_confirm_pre") ++ " ")
    ++ ((author |> Author.name) ++ " ")
    ++ t("window_confirm_post"),
    () => {
      send(BeginDeleting);
