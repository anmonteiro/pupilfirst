...
[%%raw {js|import "./TopicsShow__PostEditor.css"|js}];
...
let savePost =
  ...
  if (body != "") {
    ...
    | Some(post) =>
      ...
      let variables =
        UpdatePostQuery.makeVariables(~id=postId, ~body, ~editReason?, ());
      ...
      <div className="flex w-full">
        <div
          ...
          <div>
            ...
            {if (editing) {
               <input
                 ...
                 onChange={event => {
                   let reason = React.Event.Form.target(event)##value;
                   switch (reason) {
                   | "" => setEditReason(None)
          ...
          <div className="flex justify-end pt-3">
            ...
            {let newPostNumber =
               if (replies |> ArrayUtils.isNotEmpty) {
                 (replies |> Post.highestPostNumber) + 1;
               } else {
                 2;
               };
             <button
