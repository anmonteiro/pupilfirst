...
[@mel.module "./images/target-complete-mark-icon.svg"]
external markIcon: string = "default";
[@mel.module "./images/target-complete-link-icon.svg"]
external linkIcon: string = "default";
[@mel.module "./images/target-complete-quiz-icon.svg"]
external quizIcon: string = "default";
let str = React.string;
let t = I18n.t(~scope="components.CurriculumEditor__TargetDetailsEditor");
let ts = I18n.ts;
type nonrec methodOfCompletion =
  | Evaluated
  | VisitLink
  | TakeQuiz
  | MarkAsComplete;
type nonrec evaluationCriterion = (int, string, bool);
type nonrec state = {
  title: string,
...
let reducer = (state, action) =>
  ...
  | SaveTargetDetails(targetDetails) =>
    ...
    let checklist =
      if (targetDetails.checklist |> ArrayUtils.isNotEmpty) {
        targetDetails.checklist;
      } else {
        defaultChecklist;
      };
    let quiz =
      if (targetDetails.quiz |> ArrayUtils.isNotEmpty) {
        targetDetails.quiz;
      } else {
  ...
  | RemoveQuizQuestion(id) =>
    let quiz =
      state.quiz |> Js.Array.filter(~f=q => QuizQuestion.id(q) != id);
    {...state, quiz, dirty: true};
  ...
  | AddNewChecklistItem => {
      ...state,
      checklist:
        Js.Array.concat(~other=[|ChecklistItem.longText|], state.checklist),
      dirty: true,
    }
                     ...
                     targetGroupId => {
                       ...
                       let targetGroupsInSameLevel =
                         ...
                         |> Js.Array.filter(~f=tg =>
                              TargetGroup.levelId(tg) == levelId
                            )
                         |> Js.Array.map(~f=TargetGroup.id);
                       targets
                       ...
                       |> Js.Array.filter(~f=target =>
                            ...
                            |> Js.Array.includes(
                                 ~value=Target.targetGroupId(target),
                               )
                          )
                       |> Js.Array.filter(~f=target =>
...
let deSelectPrerequisiteTarget = (send, state, target) => {
  let updatedPrerequisites =
    state.prerequisiteTargets
    |> Js.Array.filter(~f=targetId => targetId != Target.id(target));
  send(UpdatePrerequisiteTargets(updatedPrerequisites));
};
module SelectablePrerequisiteTargets = {
  type nonrec t = Target.t;
...
let prerequisiteTargetEditor = (send, eligiblePrerequisiteTargets, state) => {
  ...
  if (eligiblePrerequisiteTargets |> ArrayUtils.isNotEmpty) {
    <div className="mb-6">
      <label
...
let selectEvaluationCriterion = (send, state, evaluationCriterion) => {
  let updatedEvaluationCriteria =
    Js.Array.concat(
      ...
      ~other=state.evaluationCriteria,
    );
...
module MultiSelectForEvaluationCriteria =
  MultiselectInline.Make(SelectableEvaluationCriterion);
let evaluationCriteriaEditor = (state, evaluationCriteria, send) => {
  let selected =
    ...
    |> Js.Array.map(~f=SelectableEvaluationCriterion.make);
  let unselected =
    evaluationCriteria
    |> Js.Array.filter(~f=ec =>
         !(
           state.evaluationCriteria
           |> Js.Array.includes(~value=EvaluationCriterion.id(ec))
         )
       )
    |> Js.Array.map(~f=SelectableEvaluationCriterion.make);
  <div id="evaluation_criteria" className="mb-6">
    ...
    <div className="ml-6">
      {if (validNumberOfEvaluationCriteria(state)) {
         React.null;
       } else {
...
let updateLinkToComplete = (send, event) =>
  send(UpdateLinkToComplete(React.Event.Form.target(event)##value));
let updateCompletionInstructions = (send, event) =>
  send(UpdateCompletionInstructions(React.Event.Form.target(event)##value));
...
let linkEditor = (state, send) =>
  <div className="mb-6">
    ...
    <div className="ml-6">
      ...
      />
      {if (state.linkToComplete |> UrlUtils.isInvalid(false)) {
         <School__InputGroupError
           message={t("enter_valid_link")}
           active=true
         />;
       } else {
         React.null;
...
let unselectedTargetGroups = (levels, targetGroups, targetGroupId) =>
  targetGroupId->(
                   Belt.Option.mapWithDefault(targetGroups, tgId =>
                     targetGroups
                     |> Js.Array.filter(~f=t =>
                          t
                          |> TargetGroup.id != tgId
                          && !(t |> TargetGroup.archived)
                        )
                   )
                 )
  |> Js.Array.map(~f=t =>
    ...
    <div className="mb-6">
      ...
      <div id="method_of_completion" className="flex -mx-2 pl-6">
        {[|`MarkAsComplete, `VisitLink, `TakeQuiz|]
         |> Js.Array.mapi(~f=(methodOfCompletion, index) =>
...
let quizEditor = (state, send) =>
  <div>
    ...
    <div className="ml-6">
      ...
      {state.quiz
       |> Js.Array.mapi(~f=(quizQuestion, index) =>
...
let doRequiredStepsHaveUniqueTitles = checklist => {
  let requiredSteps =
    checklist |> Js.Array.filter(~f=item => !(item |> ChecklistItem.optional));
  requiredSteps
  |> Js.Array.map(~f=ChecklistItem.title)
  |> Js.Array.map(~f=String.trim)
  |> ArrayUtils.distinct
...
let updateTarget = (target, state, send, updateTargetCB, targetGroupId, event) => {
  ...
  let variables =
    UpdateTargetQuery.makeVariables(
      ~id,
      ~targetGroupId,
      ~title=state.title,
      ~role,
       ...
       <DisablingCover message={ts("saving")} disabled={state.saving}>
         <div className="mt-2">
           <div className="max-w-3xl mx-auto px-3">
             ...
             {switch (state.methodOfCompletion) {
              | Evaluated =>
                <div className="mb-6">
                  ...
                  <HelpIcon
                    className="ml-1" link={t("target_checklist.help_url")}>
                  ...
                  <div className="ml-6 mb-6">
                    {state.checklist
                     |> Js.Array.mapi(~f=(checklistItem, index) => {
                          let moveChecklistItemUpCB =
                            if (index > 0) {
                              Some(() => send(MoveChecklistItemUp(index)));
                            } else {
                              None;
                            };
                          let moveChecklistItemDownCB =
                            if (index != Js.Array.length(state.checklist) - 1) {
                              Some(
                          ...
                          <CurriculumEditor__TargetChecklistItemEditor
                            ...
                            updateChecklistItemCB={newChecklistItem =>
                              send(
                                UpdateChecklistItem(index, newChecklistItem),
                              )
                            }
                     ...
                     |> React.array}
                    {if (ArrayUtils.isEmpty(state.checklist)) {
                       <div
                    ...
                    {if (Js.Array.length(state.checklist) >= 25) {
                       <div
                         className="border border-orange-500 bg-orange-100 text-orange-800 px-2 py-1 rounded my-2 text-sm text-center">
                         <i className="fas fa-info-circle mr-2" />
             ...
             {if (targetEvaluated(state.methodOfCompletion)) {
                React.null;
              } else {
             ...
             <div className="mb-6">
               ...
               <HelpIcon
                 link={t("completion_instructions.help_url")}
                 className="ml-1">
               ...
               <div className="flex items-center shrink-0">
                 ...
                 <div
                   ...
                   {[|TargetDetails.Live, Archived, Draft|]
                    |> Js.Array.mapi(~f=(visibility, index) =>
                         <button
                           key={index |> string_of_int}
