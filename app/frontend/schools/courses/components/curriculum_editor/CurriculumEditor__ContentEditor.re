...
let reducer = (state, action) =>
  ...
  | RemoveContentBlock(contentBlockId) => {
      ...
      contentBlocks:
        state.contentBlocks
        |> Js.Array.filter(~f=contentBlock =>
             contentBlock |> ContentBlock.id != contentBlockId
           ),
      dirtyContentBlockIds:
        state.dirtyContentBlockIds->(Belt.Set.String.remove(contentBlockId)),
    }
  ...
  | SetDirty(contentBlockId, dirty) =>
    let operation =
      if (dirty) {Belt.Set.String.add} else {Belt.Set.String.remove};
    {
      ...state,
      dirtyContentBlockIds:
...
let editor =
  ...
  let removeContentBlockCB =
    if (numberOfContentBlocks > 1) {
      Some(removeContentBlock(send));
    } else {
      None;
    };
  <div className="mt-2">
     ...
     |> Array.mapi((index, contentBlock) => {
          let moveContentBlockUpCB =
            if (index == 0) {
              ...
              Some(moveContentBlockUp(send));
            };
          let moveContentBlockDownCB =
            if (index + 1 == numberOfContentBlocks) {
              None;
            } else {
              Some(moveContentBlockDown(send));
            };
          let isDirty =
            state.dirtyContentBlockIds
