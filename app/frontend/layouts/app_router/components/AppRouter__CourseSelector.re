...
let contents = (courses, currentCourse, coursePage, selectedPage) =>
  Js.Array.map(
    ~f=
      course => {
        let nextPage =
          if (Page.canAccessPage(coursePage, course)) {
            coursePage;
          } else {
            Page.Curriculum;
          };
        ...
        if (Page.useSPA(
          ...
          <a key={Course.id(course)} href className=classes>
            {Course.name(course)->str}
          </a>;
        };
      },
    Js.Array.filter(
      ~f=
        course =>
          Course.id(course) != Course.id(currentCourse)
          && !Course.ended(course),
      courses,
    ),
  );
