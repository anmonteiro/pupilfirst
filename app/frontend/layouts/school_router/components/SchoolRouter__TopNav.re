...
let showUserLink = () => [|renderUserLink("power-off", "/users/sign_out")|];
...
let showUser = user =>
  <div>
    <div className="flex w-full items-center  rounded-md">
      <div
        className="flex items-center justify-center rounded-full text-center flex-shrink-0">
        {User.avatarUrl(user)
         ->(
...
let renderPrimaryPageLink = (courseId, primaryPage, secondaryPage) =>
  <div className="flex items-center space-x-2">
    <Link
      ...
      href={
        (({js|/school/courses/|js} ++ courseId) ++ {js|/|js}) ++ primaryPage
      }>
      primaryPage->str
    </Link>
...
let make = (~courses, ~currentUser) =>
  <div
    className="flex justify-between items-center p-4 bg-white border-b flex-1">
    <div
      className="flex items-center space-x-2 text-sm font-semibold capitalize">
