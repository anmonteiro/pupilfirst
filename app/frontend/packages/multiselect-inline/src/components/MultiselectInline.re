[%%raw {js|import "./MultiselectInline.css"|js}];
...
module Make = (Selectable: Selectable) => {
  let search = (searchString, selections) =>
    (
      selections
      |> Js.Array.filter(~f=selection =>
           selection
           |> Selectable.searchString
           |> String.lowercase_ascii
           |> Js.String.includes(
                ~search=searchString |> String.lowercase_ascii,
              )
         )
    )
    ->(
        ...
        {if (selected |> Array.length > 0) {
           ...
           |> Array.mapi((index, selected) =>
                <span
                  ...
                  className={
                    "inline-flex items-center font-semibold text-xs mb-2 mr-2 rounded-full overflow-hidden "
                  ...
                  <button
                    className={
                      "inline-flex shrink-0 px-2 py-1 text-sm border-l-0 rounded-r items-center text-gray-800 hover:bg-gray-50 hover:text-red-500 focus:outline-none focus:bg-gray-50 focus:text-red-500 "
             ...
             {if (showSearchForm) {
                <div className="px-3 pt-3 pb-2">
                  <input
                    ...
                    placeholder
                    onChange={event =>
                      onChange(React.Event.Form.target(event)##value)
                    }
                  />
                </div>;
              } else {
