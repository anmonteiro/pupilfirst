...
let reducer = (state, action) =>
  ...
  | FinishDeleting(certificate) => {
      ...
      certificates:
        Js.Array.filter(
          ~f=c => Certificate.id(c) != Certificate.id(certificate),
          state.certificates,
        ),
      deleting: false,
    }
...
let addCertificate = (state, send, certificate) => {
  let newCertificates =
    Js.Array.concat(~other=[|certificate|], state.certificates);
...
let make = (~course, ~certificates, ~verifyImageUrl, ~canBeAutoIssued) => {
  ...
  <DisablingCover
    containerClasses="min-h-full bg-gray-50 w-full"
    ...
    <div className="flex">
      ...
      <div className="flex flex-1 flex-col">
        ...
        {if (state.certificates |> ArrayUtils.isEmpty) {
           <div
