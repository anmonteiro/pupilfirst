 ...
 |js}
  ...
  let reducer = (state: state, action) =>
    ...
    | UpdateRevokedCertificate(certificate) => {
        ...
        student: {
          ...state.student,
          issuedCertificates:
            state.student.issuedCertificates
            ->(
                Js.Array.map(~f=ic =>
                  if (IssuedCertificate.id(certificate)
                      == IssuedCertificate.id(ic)) {
                    certificate;
                  } else {
                    ic;
                  }
    ...
    | SetIssuing => {...state, issuing: true}
    | ClearIssuing => {...state, issuing: false}
    | SetRevoking => {...state, revoking: true}
    | ClearRevoking => {...state, revoking: false}
  ...
  let reActivateStudent = (id, send, event) => {
    ...
    |> Js.Promise.then_((response: ReActivateStudentQuery.t) => {
         if (response.reActivateStudent.success) {
           send(ReActivateStudent);
         } else {
           send(ClearSaving);
         };
         Js.Promise.resolve();
             ...
             | Some(data) =>
               send(
                 UpdateRevokedCertificate(
                   IssuedCertificate.makeFromFragment(data),
                 ),
               )
  ...
  let issueNewCertificate = (studentId, state, send, event) => {
    ...
    |> Js.Promise.then_((response: IssueCertificateQuery.t) => {
         ...
         switch (data) {
         | Some(data) =>
           send(AddNewCertificate(IssuedCertificate.makeFromFragment(data)))
  ...
  let issueButtonIcons = issuing =>
    if (issuing) {"fas fa-spinner fa-spin"} else {"fas fa-certificate"};
  let showIssuedCertificates = (student, state, send) => {
    let issuedCertificates = student.issuedCertificates;

    if (issuedCertificates->ArrayUtils.isEmpty) {
                   ...
                   <div className="flex flex-col">
                     <div>
                       ...
                       <span className="text-xs ml-2">
                         {IssuedCertificate.createdAt(ic)
                     ...
                     <div>
                       ...
                       <span className="text-xs ml-2">
                         {IssuedCertificate.issuedBy(ic)->str}
                       </span>
                     </div>
                   </div>
  ...
  let hasLiveCertificate = student =>
    ...
    ->(
        Js.Array.find(~f=ic =>
          IssuedCertificate.revokedAt(ic)->Belt.Option.isNone
        )
      )
    ->Belt.Option.isSome;
  [@react.component]
  let make = (~studentData, ~studentId, ~certificates) => {
    ...
    <div className="pt-5 pb-10">
                ...
                <div className="flex flex-col mt-2">
                  ...
                  <div className="flex items-end mt-2">
                           ...
                           Js.Array.map(~f=certificate =>
                             <option
                               ...
                               {let name = Certificate.name(certificate);
                                ...
                                (
                                  if (Certificate.active(certificate)) {
                                    name ++ " (" ++ t("active_label") ++ ")";
                                  } else {
                                    name;
                                  }
                    ...
                    <button
                      ...
                      disabled={
                        state.issuing || state.selectedCertificateId == "0"
                      }
      ...
      <div className="p-4 bg-red-50 border-red-300 rounded mt-12">
        ...
        <div className="mt-2">
          {if (Belt.Option.isNone(state.student.droppedOutAt)) {
             <button
             ...
             <button
               ...
               <FaIcon classes={submitButtonIcons(state.saving)} />
               <span className="ml-2">
                 {t("re_activate_student.button")->str}
               </span>
             </button>;
           }}
        </div>
       ...
       setState(_ =>
         Loaded({
           student: {
             ...
             droppedOutAt:
               response.student.droppedOutAt
               ->(Belt.Option.map(DateFns.decodeISO)),
             issuedCertificates:
               response.student.issuedCertificates
               ->(Js.Array.map(~f=IssuedCertificate.makeFromFragment)),
           },
           certificates:
             response.student.course.certificates
             ->(Js.Array.map(~f=Certificate.makeFromFragment)),
