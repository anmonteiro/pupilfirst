[%%raw {js|import "./PfIcon__Example.css"|js}];
[@mel.scope ("window", "navigator", "clipboard")]
...
module Example = {
  ...
  let search = searchString => {
    ...
    | searchString =>
      icons
      |> Js.Array.filter(~f=icon =>
           icon
           |> String.lowercase_ascii
           |> Js.String.includes(~search=searchString)
         )
      |> copyAndSort(String.compare)
    };
  };
  let onChange = (setSearchString, event) => {
    let searchString = React.Event.Form.target(event)##value;
          ...
          {switch (search(searchString)) {
           ...
           | resultIcons =>
             ...
             |> Array.map(icon => {
                  ...
                  <div
                    key=icon
                    className="flex items-center p-4 shadow bg-white rounded-md">
