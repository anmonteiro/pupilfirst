...
let buttonContents = (formState, checklist) => {
  ...
  let text =
    (
      ...
      | Ready =>
        if (checklist |> ArrayUtils.isEmpty) {
          tr("complete");
        } else {
          tr("submit");
        }
...
let setAttaching = (send, bool) =>
  send(if (bool) {SetAttaching} else {SetReady});
...
let make = (~target, ~addSubmissionCB, ~preview, ~checklist) => {
  ...
  <div
    className="bg-gray-50 p-4 my-4 border rounded-lg" id="submission-builder">
    <DisablingCover
      disabled={isBusy(state.formState)}
      message={statusText(state.formState)}>
      {if (state.checklist |> ArrayUtils.isEmpty) {
