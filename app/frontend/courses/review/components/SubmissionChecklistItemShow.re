...
let statusIcon = (updateChecklistCB, status) =>
  ...
  | (None, Passed) =>
    <div
      className="flex items-center space-x-2 text-xs bg-green-100 px-1 py-px mt-1">
      <PfIcon
  ...
  | (None, Failed) =>
    <div
      className="flex items-center space-x-2 text-xs bg-red-100 px-1 py-px mt-px">
      <PfIcon
        className="if i-times-square-solid text-red-500 text-base bg-white"
      />
...
let statusButtonIcon = bool =>
  if (bool) {"if i-times-square-solid text-base if-fw"} else {
...
let statusButton = (index, status, callback, checklist) =>
  <div className="mt-2">
    <button
      ...
      className={
        "border border-gray-500 rounded focus:ring-2 focus:ring-offset-2 focus:ring-focusColor-500 "
      ...
      <span className="p-2">
        (
          if (status == ChecklistItem.Failed) {
            tr("mark_correct");
          } else {
            tr("mark_incorrect");
          }
...
let cardBodyClasses = pending =>
  "pl-7 md:pl-8 " ++ (if (pending) {""} else {"rounded-b"});
[@react.component]
    ...
    <div className="text-sm font-medium flex items-center justify-between ">
      <div className="flex flex-1 items-start space-x-2">
        <div className="flex items-start">
          <PfIcon
            className={kindIconClasses(ChecklistItem.result(checklistItem))}
          />
          <MarkdownBlock
