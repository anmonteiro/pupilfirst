[%%raw {js|import "./CoursesReport__Overview.css"|js}];
    ...
    <div
      ...
      <div className="ml-4">
        <p className="text-sm text-gray-600 font-semibold mt-1">
          {t(
             ~variables=[|
               ("targetsCount", string_of_int(incompleteTargets)),
             |],
             "incomplete_targets",
           )
           |> str}
        ...
        <p className="text-sm text-gray-600 font-semibold mt-1">
          {t(
             ~variables=[|
               ("targetsCount", string_of_int(targetsPendingReview)),
             |],
...
let averageGradeCharts =
  ...
  |> Array.map(grade => {
       let criterion =
         StudentOverview.evaluationCriterionForGrade(
           grade,
           evaluationCriteria,
         );
       let passGrade =
         criterion
         |> CoursesReport__EvaluationCriterion.passGrade
         |> float_of_int;
           ...
           <div className="flex px-5 pt-4 text-center items-center">
             <svg
               ...
               <circle
                 ...
                 strokeDasharray={
                   StudentOverview.gradeAsPercentage(grade, criterion)
                   ++ ", 100"
                 }
                 r="16"
                 cx="16"
...
let studentLevelClasses = (levelNumber, levelCompleted, currentLevelNumber) => {
  ...
  let completed =
    if (levelCompleted) {" courses-report-overview__student-level--completed"} else {
      ""
    };
