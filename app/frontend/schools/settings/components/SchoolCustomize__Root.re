...
[%%raw {js|import "./SchoolCustomize__Root.css"|js}];
...
type action =
  ...
  | MoveLink(
      Customizations.linkId,
      SchoolCustomize__LinkComponent.kind,
      Customizations.direction,
    )
...
let headerLinks = links => {
  let (visibleLinks, dropdownLinks) =
    if (links->Js.Array.length > 4) {
      (
        links->(Js.Array.slice(~start=0, ~end_=3)),
        links->(Js.Array.slice(~start=3)),
      );
    } else {
      (links, [||]);
    };
...
let socialLinks = links =>
  ...
  | links =>
    <div className="flex flex-wrap">
      {links
       ->(
           Js.Array.map(~f=((id, _title, url, _)) =>
...
let editIcon = (additionalClasses, clickHandler, title) =>
  <button
    className={
      "cursor-pointer px-2 py-1 bg-primary-100 border border-primary-400 text-primary-500 hover:bg-primary-200 hover:border-primary-500 hover:text-primary-600 focus:bg-primary-200 focus:border-primary-500 focus:text-primary-600px rounded flex items-center "
      ++ additionalClasses
    }
    title
    ariaLabel=title
       ...
       | LinksEditor(kind) =>
         <SchoolCustomize__LinksEditor
           ...
           moveLinkCB={(id, kind, direction) =>
             send(MoveLink(id, kind, direction))
           }
           removeLinkCB={linkId => send(RemoveLink(linkId))}
           updateLinkCB={(linkId, title, url) =>
             send(UpdateLink(linkId, title, url))
           }
         />
...
let moveLink = (linkId, kind, direction, t) => {
  let similarKindLinks =
    switch (kind) {
    | SchoolCustomize__LinkComponent.HeaderLink =>
      Customizations.filterLinks(~header=true, t)
  ...
  let linkIndex =
    ...
    ->(
        Js.Array.map(~f=l =>
          switch (l) {
          | Customizations.HeaderLink(id, _, _, _)
          | FooterLink(id, _, _, _)
          | SocialLink(id, _, _) => id
          }
        )
      )
    ->(Js.Array.indexOf(~value=linkId));
  let swapedLinks =
    similarKindLinks->(
                        switch (direction) {
  ...
  let updatedLinks =
    differentKindLinks->(Js.Array.concat(~other=swapedLinks));
  {...t, links: updatedLinks};
};
let reducer = (state, action) =>
  ...
  | UpdateLink(linkId, title, url) => {
      ...state,
      customizations:
        state.customizations |> Customizations.updateLink(linkId, title, url),
    }
  | RemoveLink(linkId) => {
      ...state,
      customizations:
      ...
      <div className="mt-3 w-1/2 max-w-sm">
        <div className="bg-gray-300 rounded-t-lg h-12 flex items-end">
          <div className="w-full flex items-center pr-3">
            ...
            <div className="h-3 w-3 rounded-full bg-gray-500 ml-2" />
            <div
              className="p-3 ml-4 bg-gray-50 rounded-t-lg flex items-center">
              <img
                src={
                  state.customizations
                  |> Customizations.icon
