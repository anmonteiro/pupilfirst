...
let reducer = (state, action) =>
  ...
  | DeselectTag(tag) => {
      ...
      selectedTags:
        state.selectedTags
        |> Js.Array.filter(~f=t => t |> Tag.id != Tag.id(tag)),
    }
...
let unselected = (allTags, selectedTags) => {
  ...
  allTags
  |> Js.Array.filter(~f=t => !(selectedTagIds |> Array.mem(t |> Tag.id)));
};
...
let createCourseExport = (state, send, course, event) => {
  ...
  let variables =
    CreateCourseExportQuery.makeVariables(
  ...
  <div className="bg-gray-50 h-full" key="School admin coaches course index">
    {if (state.drawerOpen) {
       <SchoolAdmin__EditorDrawer
         ...
         closeButtonTitle={t("close_export_form")}>
         <div className="mx-auto bg-white">
           <div className="max-w-2xl pt-6 px-6 mx-auto">
             <h5
               className="uppercase text-center border-b border-gray-300 pb-2">
               ...
               <div id="targets_filter" className="flex -mx-2">
                 ...
                 <div className="w-1/2 px-2">
                   <button
                     ...
                     <div className="mt-1">
                       {t("reviewed_only_targets_label")->str}
                     </div>
                   </button>
                 </div>
               </div>
               ...
               <div id="inactive_students_filter" className="flex -mx-2">
                 <div className="w-1/2 px-2">
                   <button
                     ...
                     className={toggleChoiceClasses(
                       !state.includeInactiveStudents,
                     )}>
                     <i className="fas fa-list" />
                     <div className="mt-1">
                       {t("active_students_label")->str}
                     </div>
                   </button>
                 </div>
                 <div className="w-1/2 px-2">
                   <button
                     onClick={_event =>
                       send(SetIncludeInactiveStudents(true))
                     }
                     className={toggleChoiceClasses(
                       state.includeInactiveStudents,
                     )}>
               ...
               <button
                 ...
                 {if (state.saving) {
                    <span>
                      ...
                      <span className="ml-2">
                        {t("create_button_active_label")->str}
                      </span>
                    </span>;
                  } else {
    ...
    <div className="flex-1 flex flex-col">
      ...
      {if (state.courseExports |> ArrayUtils.isEmpty) {
         <div
           className="flex justify-center border rounded p-3 italic mx-auto max-w-2xl w-full">
                         ...
                         <div className="flex flex-1 justify-between">
                           <div className="pt-4 pb-3 px-4">
                             ...
                             <div
                               ...
                               {if (courseExport->CourseExport.reviewedOnly) {
                                  <span
                               ...
                               {if (courseExport->CourseExport.includeInactiveStudents) {
                                  <span
                                    className="px-2 py-1 border rounded bg-orange-100 text-orange-600 mt-1 mr-1">
