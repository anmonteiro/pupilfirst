...
type nonrec selected = bool;
type nonrec item = (key, value, selected);
  ...
  let filteredList =
    ...
    | someList =>
      ...
      |> List.filter(((_, value, _)) =>
           Js.String.includes(
             ~search=String.lowercase_ascii(searchString),
           ...
           {if (nonSelectedList |> List.length > 3) {
              <div className="px-3 pt-3 pb-2">
                <input
                  ...
                  onChange={event =>
                    setSearchString(React.Event.Form.target(event)##value)
                  }
                />
              </div>;
            } else {
